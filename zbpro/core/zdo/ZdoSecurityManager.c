...
)
{
  ...
  for (cIndex = 0; cIndex < ZbStackTablesSizes(gExclusionMax); cIndex++)
  {
    /* find a free slot */
    if(freeIndex == 0xff && Cmp8BytesToZero(ZbBeeStackGlobalsParams(gaExclusionTable[cIndex])))
    {
      freeIndex = cIndex;
    }

    /* already in table */
    if(IsEqual8Bytes(ZbBeeStackGlobalsParams(gaExclusionTable[cIndex]),pIeeeAddress))
    {
      return TRUE;
    }
  }  /* End of For loop */
  ...
  Copy8Bytes(ZbBeeStackGlobalsParams(gaExclusionTable[freeIndex]),pIeeeAddress);
  return TRUE;
}
...
)
{
  ...
  {
    if (IsEqual8Bytes(ZbBeeStackGlobalsParams(gaExclusionTable[i]),pIeeeAddress))
    {
      BeeUtilZeroMemory(ZbBeeStackGlobalsParams(gaExclusionTable[i]), sizeof(zbIeeeAddr_t));
      return TRUE;
    }
  }
...
* create a transport-key command packet (see 053474r20 ZigBee Pro spec sub-clause
* 4.4.9.2).
*
* Interface assumptions:
...
)
{
  ...
  index_t index = SSP_ApsGetIndexFromIEEEaddress(ApsmeGetRequest(gApsTrustCenterAddress_c));
  uint8_t updatesToSend = 1;  
  ...
  /*
    Check if the aps link key for the destination is global or unique. The first
    key in gaApsKeySet is the global key(TC link key).
  */
  if(!ZbApsPrivateData(gOverrideApsFrameEncryptionSettings_c.overrideUpdateDeviceEncryption)&& 
     devKeyPairSetEntry.iKey == 0x00)
  {
    updatesToSend = 2;
    ZbApsPrivateData(gUpdateDeviceSecUnsecTransmit_c) = 2;
  }
...
)
{
  ...
  */
  ...
  pZdoApsmeMsg->msgType = gApsmeRemoveDeviceReq_c;
...
#if gHighSecurity_d
/*******************************************************************************
* ZDO_APSME_Authenticate_request (053474r20 ZigBee Pro spec section 4.4.8.1.2)
*
...
)
{
  ...
  */
  switch (pUpdateIndication->status)
  {
    ...
    */
    case gDeviceLeft_c:
#if gApsLinkKeySecurity_d && !gApsRemoveLinkKeyOnDeviceLeave_d
...
{
  if(ZbNwkPrivateData(gKeepNeighborInformation))
  {
    return;
  }
#if gRnplusCapability_d
  ...
  (void)aNwkAddr;
  ZbNwkPublicData(gSecClearDeviceDataEvent) = onLostChildBroadcast_c;
}
#endif
...
#if gStandardSecurity_d || gHighSecurity_d
/************************************************************************************
* 053474r20 ZigBee Pro spec - Sec. 4.4.2.1 APSME-ESTABLISH-KEY.request
...
)
{
  ...
  if (pSkkeData->sendTrasnportKey && (pApsmeCnf->status == gZbSuccess_c))
  {
    ...
    */
    ...
    if (pSecurityMaterial)
    {
      ...
      /*
        ZDO Must send the APSME transport key request to be deliver to the newly joined
        device.
      */
      if (pSkkeData->useParent)
      {
  ...
  */
  ...
  {
    ZdoNwkMng_SaveToNvm(zdoNvmObject_ApsLinkKeySet_c, NULL);
  }
  else
  {
...
)
{
  ...
  */
  MSG_Queue(&ZbZdoPrivateData(gUpdateDeviceQueue), pUpdateDevice);
...
)
{
  ...
  */
  ...
  /* If the current device must leave, the device address of the leave request 
  must be all zeros not the device ieee address */
  
  if(!Cmp8Bytes(pRemoveDeviceInd->aChildAddress, NlmeGetRequest(gNwkIeeeAddress_c)))
  {
  ...
  if (pNeighborTableEntry)
  {
    ...
    */
    ...
    if (pNeighborTableEntry->deviceProperty.bits.relationship == gPreviousChild_c)
    {
      MSG_Free(pZdoNlmeMsg);
      return gZbSuccess_c;
    }
  }
...
* RESPOND_REJECT. 053474r20 ZigBee Pro spec section 4.4.8.3.3
*
* Interface assumptions:
...
)
{
  ...
  /* Check if the Random challenge sent by the initiator is accepted acording to
     the procedure 053474r20 ZigBee Pro spec section B3.2. */
...
)
{
  ...
  ZbTMR_StopTimer(pEntityAuthentication_Data->EA_TimerID);
  ZbTMR_FreeTimer(pEntityAuthentication_Data->EA_TimerID);
  ...
  */
  ...
  if (pAuthenticateCnf->status == gZbSuccess_c)
  {
    ...
    ZdoNwkMng_SaveToNvm(zdoNvmObject_NwkData_c, NULL);
  }
...
)
{
  ...
  {
     zbAESKey_t defaultGlobalTCLinkKey = {mDefaultGlobalTrustCenterLinkKey_c};
     ApsmeSetRequest(gApsTrustCenterLinkKey_c, &defaultGlobalTCLinkKey);
  } 
  ...
  {
      /* set also the entry 1*/  
    zbAESKey_t defaultEntry1LinkKey = {mDefaultZllCertificationPreinstalledLinkKey_c};
    pKey = (uint8_t *)defaultEntry1LinkKey;
    (void)KeySet_SetTableEntry(mDefaultZllCertPreinstalledLinkKeyIndex_c, (zbAESKey_t *)pKey);
  } 
#endif  
...
void ZDO_SecClearAPSIncomingCounters(void)
{
  uint8_t i;  /* Loop counter. */
  zbApsDeviceKeyPairSet_t devKeyPairSetEntry;

  /* Start from the beginning and walk the whole list */

  /* Make sure to update each entry of the array. */
  for (i = 0; i < ApsmeGetRequest(gApsDeviceKeyPairCount_c); i++)
  {
    (void)DevKeyPairSet_GetTableEntry(i, &devKeyPairSetEntry);
    /* Skip the unused ones if theres any. */
    ...
    /* Clear the counters. */
    BeeUtilZeroMemory((void*)&devKeyPairSetEntry.IncomingFrameCounter, sizeof(zbFrameCounter_t));
    (void)DevKeyPairSet_SetTableEntry(i, &devKeyPairSetEntry);
  }
}
#endif
...
#if gComboDeviceCapability_d
  ZbBeeStackNwkGlobals(gTrustCenter) = FALSE;
#endif  /* gComboDeviceCapability_d  */

  if (NlmeGetRequest(gDevType_c) == gCoordinator_c)
  {
#if gComboDeviceCapability_d
   ZbBeeStackNwkGlobals(gTrustCenter) = TRUE;
#endif  /* gComboDeviceCapability_d  */
   ZdoSetPrimaryTrustCenterOn( ZbZdoPublicData(gaServerMask)); /* Trust center bit enable. */ /* Trust center bit enable. */
  }
}
#endif
