...
)
{
	if (ZbZdoPrivateData(gDiscoveryCacheResponse))
	  Copy2Bytes(pAddress, ZbZdoPrivateData(gDiscoveryCacheSuccess.aNwkAddress));
...
{
	if (ZbZdoPrivateData(gDiscoveryCacheResponse))
				Copy2Bytes(aNwkAddress, ZbZdoPrivateData(gDiscoveryCacheSuccess.aNwkAddress));
...
{
	if (ZbZdoPrivateData(gDiscoveryCacheResponse))
				Copy2Bytes(aAddress, ZbZdoPrivateData(gDiscoveryCacheSuccess.aNwkAddress));
...
)
{
	...
	if (ZbZdoPrivateData(gDiscoveryCacheResponse))
			Copy2Bytes(aDestAddr, ZbZdoPrivateData(gDiscoveryCacheSuccess.aNwkAddress));
  ...
  */
  else if (pRequest->ScanDuration == 0xfe)
  {
    ...
    /*
      ...
      the state machine.
    */
    if (ZdoGetNwkManagerBitMask( ZbZdoPublicData(gaServerMask))&&(FA_GetNwkManagerState() == gInitState_c))
    {
      /*
      ...
      */
//replaced ASL_Mgmt_NWK_Update_req
#if 0        
      ...
      ZDP_Mgmt_NWK_Update_req(NlmeGetRequest(gNwkShortAddress_c), /* To Us. */
                              pRequest->aScanChannels,  /* Same Channel list ot be use. */
                              0xfe,  /* Do a channel change. */
                              pRequest->ExtraData.NwkManagerData.nwkUpdateId);  /* No scan duration, is a channel change. */
    }
