...
)
{
  ...
  if (!(IsSelfNwkAddress(pMessageComingIn->aNwkAddrOfInterest)))
  {
...
#endif
    {
      ...
      {
        pStore = &ZbZdoPrivateData(gaDiscoveryStoreTable[cIndex]);
...
)
{
  ...
  for( cIndex=0; cIndex < numOfActiveEndPoints ; cIndex++ )
  {
    ...
    if (!IsProfileIdMatched(pSimpledesc, pMessageComingIn->aProfileId))
   {
...
)
{
  ...
  FLib_MemSet(&ZbBeeStackGlobalsParams(gUserDesc),' ', sizeof(ZbBeeStackGlobalsParams(gUserDesc)));
  /* including the length of the user descriptor is copied here */
  FLib_MemCpy(&ZbBeeStackGlobalsParams(gUserDesc),&pMessageComingIn->descriptor, sizeof(pMessageComingIn->descriptor) );
  Copy2Bytes(pMessageGoingOut->aNwkAddrOfInterest, NlmeGetRequest(gNwkShortAddress_c));
...
)
{
  ...
  */
  ...
  BeeUtilZeroMemory(&ZbZdoPrivateData(gaDiscoveryStoreTable[cIndex]), sizeof(discoveryStoreTable_t));
...
)
{
  ...
  Set2Bytes(pMessageGoingOut->aBindingTableEntrys, 
            ZDP_GetBindingCacheEntryListByAddressAndStartIndex(aIeeeAddr, 0, (uint8_t *)pMessageGoingOut->aBindingTableList,&listSize));
...
)
{
  ...
  if (!ZdoGetPrimaryBindingTableCacheBitMask( ZbZdoPublicData(gaServerMask)) || Cmp8BytesToZero(pMessageComingIn->aOldAddress))
    return (MbrSizeof(zbReplaceDeviceResponse_t, status));

  for (auxIndex = 0; auxIndex < ZbStackTablesSizes(gMaximumBindingCacheTableList); auxIndex++)
  {
    ...
    if (cIndex != gZdoNoEntry_c)
    {
      ...
      else
      {
        ...
        {
          pPtr= &ZbZdoPrivateData(gaBindingCacheTable[auxIndex]);
...
{
  if (!ZdoGetBackupBindingTableCacheBitMask( ZbZdoPublicData(gaServerMask)))
  {
    pMessageGoingOut->status = gZdoNotSupported_c;
...
)
{
  ...
  if (cIndex != gZdoNoEntry_c)
  {
    ZdpBindSetFree(ZbZdoPrivateData(gaBindingCacheTable[cIndex]));
    pMessageGoingOut->status = gZbSuccess_c;
    return (MbrSizeof(zbRemoveBackupBindEntryResponse_t, status));
  }
  else
...
)
{
  ...
  */
  ...
  if ((cIndex + TwoBytesToUint16(pMessageComingIn->BindingTableListCount)) > ZbStackTablesSizes(gMaximumBindingCacheTableList))
  {
    /*
    ...
    */
    pMessageGoingOut->status = gZdoTableFull_c;
    Set2Bytes(pMessageGoingOut->entryCount,ZbStackTablesSizes(gMaximumBindingCacheTableList));
...
)
{
  ...
  */
  ...
  Swap2BytesArray(pMessageComingIn->startIndex);
#endif  
  if (TwoBytesToUint16(pMessageComingIn->startIndex) > ZbStackTablesSizes(gMaximumBindingCacheTableList))
  {
    pMessageGoingOut->status = gZdoNoEntry_c;
    return (MbrOfs(zbRecoverBindTableResponse_t, aBindingTableList[0]));
  }
  ...
  Set2Bytes(pMessageGoingOut->aBindingTableEntries,
            ZDP_GetBindingCacheEntryListByAddressAndStartIndex(aNullAddress, TwoBytesToUint16(pMessageComingIn->startIndex), (uint8_t *)pMessageGoingOut->aBindingTableList,&listSize));
  ...
  if (TwoBytesToUint16(pMessageGoingOut->aBindingTableEntries) >= (listSize / (uint8_t)sizeof(zbApsmeBindEntry_t)))
    Set2Bytes(pMessageGoingOut->aBindingTableListCount, listSize / (uint8_t)sizeof(zbApsmeBindEntry_t));
  else
...
cIndex = TwoBytesToUint16(pMessageComingIn->StartIndex);        
  ...
  if ((cIndex + TwoBytesToUint16(pMessageComingIn->SourceTableListCount)) > ZbStackTablesSizes(gMaximumDevicesHoldingBindingInfo))
  {
    /*
      If this exceeds its table size it shall fill in as many entries
  ...
  ZbZdoPrivateData(gDeviceHoldingItsOwnBindingInfo_Counter) = 0;
  for (auxIndex = 0;cIndex < limit;cIndex++,auxIndex++)
  {
    Copy8Bytes(ZbZdoPrivateData(aDevicesHoldingItsOwnBindingInfo[cIndex]),pMessageComingIn->SourceTableList[auxIndex]);
...
)
{
...
#endif
  cIndex = TwoBytesToUint16(pMessageComingIn->StartIndex);
  if ((cIndex > ZbStackTablesSizes(gMaximumDevicesHoldingBindingInfo)) ||
      (cIndex > ZbZdoPrivateData(gDeviceHoldingItsOwnBindingInfo_Counter)))
  {
    pMessageGoingOut->Status = gZdoNoEntry_c;
    return (sizeof(zbStatus_t));
  }
  ...
  pMessageGoingOut->Status = gZbSuccess_c;
  Set2Bytes(pMessageGoingOut->SourceTableEntries, ZbZdoPrivateData(gDeviceHoldingItsOwnBindingInfo_Counter));
#if (gBigEndian_c)  
  ...
  for (auxIndex = 0; cIndex < limit; cIndex++,auxIndex++)
  {
    if (auxIndex >= ZbZdoPrivateData(gDeviceHoldingItsOwnBindingInfo_Counter))
      break;

    if (cIndex >= ZbStackTablesSizes(gMaximumDevicesHoldingBindingInfo))
      break;

    Copy8Bytes(pMessageGoingOut->SourceTableList[auxIndex], ZbZdoPrivateData(aDevicesHoldingItsOwnBindingInfo[cIndex]));
  }
...
)
{
  ...
  SetClusterId(ZbZdoPrivateData(gIndex), gMgmt_NWK_Disc_req_c);
  StoreAddress( ZbZdoPrivateData(gIndex), aNwkAddress );
...
{
  index_t  cIndex;
  uint8_t skipEntries = pMessageComingIn->startIndex;
  uint8_t maxEntries;
  ...
  maxEntries = ZdpMaximumEntriesInPayLoad(MbrOfs(zbMgmtLqiResponse_t, neighbourTableList[0]),sizeof(zbNeighborTableList_t));
  
  for(cIndex = 0; cIndex < ZbStackTablesSizes(gMaxNeighborTableEntries); cIndex++)
  {
    if( pMessageGoingOut->neighbourTableListCount >= maxEntries )
    {
      break;
    }
    ...
    else if (pNeighborTable->deviceProperty.bits.relationship == gNeighborIsParent_c)
    {
      pMessageGoingOut->neighbourTableList[pMessageGoingOut->neighbourTableListCount].depth = NlmeGetRequest(gNwkDeviceDepth_c) - 1;
    }
    else if (Cmp2BytesToZero(pNeighborTable->aNetworkAddress))
    {
      /* Coordinator always at depth 0 */
      pMessageGoingOut->neighbourTableList[pMessageGoingOut->neighbourTableListCount].depth = 0;
    }
    else
    {
      /* Depth unknown for silbings */
      pMessageGoingOut->neighbourTableList[pMessageGoingOut->neighbourTableListCount].depth = NlmeGetRequest(gNwkMaxDepth_c);
    }
...
)
{
  ...
  for(cIndex = 0; cIndex < gMaxRouteTableEntries; cIndex++) 
  {
    ...
    proutingTableEntry = &ZbBeeStackNwkGlobals(gaRouteTable[cIndex]);
...
)
{
  ...
  /*
    ...
    nothing to process.
  */
  if (!ZdoGetPrimaryBindingTableCacheBitMask( ZbZdoPublicData(gaServerMask)) && !entryStatus)
  {
    pMessageGoingOut->status = gZdoInvalidRequestType_c;
  ...
  tableType = (ZdoGetPrimaryBindingTableCacheBitMask( ZbZdoPublicData(gaServerMask))? TRUE : FALSE);
...
)
{
  ...
  */
  ...
  Copy2Bytes(ZbZdoPrivateData(mgmtLeaveResponseInfo),aNwkAddress);

  if ((IsSelfIeeeAddress(pMessageComingIn->aDeviceAddress)) || (Cmp8BytesToZero(pMessageComingIn->aDeviceAddress)))
  {
    /*
    ...
    */
    ...
    ZbZdoPrivateData(gMgmtSelfLeaveMask) |= pMessageComingIn->mgmtOptions;  /* possible rejoin or remove children */
...
)
{
  ...
  */
  Copy2Bytes(ZbZdoPrivateData(aSrcAddress),aSrcAddr);

#if ((gStandardSecurity_d || gHighSecurity_d) && gApsMaxEntriesForPermissionsTable_c)
  if (!CommandHasPermission(aSrcAddr, gNetworkSettingsPermission_c))
  {
    pZdoMsg = MSG_Alloc(sizeof(nlmeZdoMessage_t));
    if(pZdoMsg)
    {
      pZdoMsg->msgData.directJoinConf.status = gZdpNotAuthorized_c;
...
)
{
  ...
  pMessageGoingOut->discoveryCacheEntries = ZbZdoPrivateData(mDiscoveryStoreTableEntrys);
  pMessageGoingOut->startIndex = startIndex;

  for (cIndex = startIndex; cIndex < ZbStackTablesSizes(gMaximumDiscoveryStoreTableSize); cIndex++)
  {
    pPtr = &ZbZdoPrivateData(gaDiscoveryStoreTable[cIndex]);
