...
/* is this entry valid? */
#define EdbIsValid(i) (ZbZdoPrivateData(maEdbTable[(i)].state))
...
{
  /* stop the timer */
  ZbTMR_StopTimer(gEndDevBindTimerID);

  /* no longer in EDB, so ZDO will not send it's bind responses here.. */
  ZbZdoPrivateData(gZdoInEndDeviceBind) = ZbZdoPrivateData(mEdbError) = FALSE;
  ZbZdoPrivateData(mEdbTicksLeft) = ZbZdoPrivateData(mEdbCurrentIndex) = 0;
  ZbZdoPrivateData(mEdbProcessTicksLeft) = 0;

  /* done, clear everything */
  #if(gInstantiableStackEnabled_d == 1)  
  BeeUtilZeroMemory(ZbZdoPrivateData(maEdbTable), sizeof(zdoEdbTable_t)*ZbStackTablesSizes(mEdbMaxEntries));
  #else
...
void ZdpEndDeviceBindStateMachine(void)
{
  ...
  if (!ZbZdoPrivateData(mEdbTicksLeft))
  {
    if(!ZbZdoPrivateData(mEdbProcessTicksLeft))
    {
      /* The 2nd EndDeviceBind.Request was not received, go through and send
      time-out response to all nodes that didn't get a response
      */
    ...
    {
      ZbZdoPrivateData(mEdbProcessTicksLeft)--;
    }
  }
  ...
  {
    --ZbZdoPrivateData(mEdbTicksLeft);
  }

  /* go through list, looking for valid entries that need attention */

#if(gInstantiableStackEnabled_d == 1)    
  pPtr = &ZbZdoPrivateData(maEdbTable[pZbZdoPrivateData->mEdbCurrentIndex]);
#else
  pPtr = &ZbZdoPrivateData(maEdbTable[mEdbCurrentIndex]);  
#endif
  ...
  switch(pPtr->state)
  {
    ...
    case mEdbStateAllocatingUnbind_c:
      if (EbdSendBindUnBind(fBind, ZbZdoPrivateData(mEdbCurrentIndex)))
      {
    ...
    case mEdbStateWaitingForStatus_c:
      if(EdbSendResponse(ZbZdoPrivateData(mEdbCurrentIndex)))
      {
        /* if all entries are done, then stop state machine */
        ++ZbZdoPrivateData(mEdbCurrentIndex);
        if(ZbZdoPrivateData(mEdbError) || ZbZdoPrivateData(mEdbCurrentIndex) >= ZbStackTablesSizes(mEdbMaxEntries))
        {
          ZdpEndDeviceBindReset();
          return;
        }
...
bool_t EdbInvalidEndPoint(index_t iEbdIndex)
{
  ...
  pPtr = &ZbZdoPrivateData(maEdbTable[iEbdIndex]);
...
zbSize_t ZdpEndDeviceBindGotRequest(zbEndDeviceBindRequest_t *pReqFrame)
{
  ...
  if(!ZbZdoPrivateData(gZdoInEndDeviceBind))
  {
    ...
    {
      ZbZdoPrivateData(mEdbError) = TRUE;
    }

    /* start up the interval timer to feed the state machine */
    ZbZdoPrivateData(mEdbTicksLeft) = mEdbTicksToExpire_c;
    ZbTMR_StartIntervalTimer(gEndDevBindTimerID, mEdbTickInterval_c, EdbTimeOutCallBack);

    /* indicate we are in binding state */
    ++ZbZdoPrivateData(gZdoInEndDeviceBind);
  }

  /* check for a match with the first EDB.request */
  else if(!ZbZdoPrivateData(mEdbError) && ZbZdoPrivateData(gZdoInEndDeviceBind) < mEdbMaxEntries_c)
  {
    ...
    if(EdbInvalidEndPoint(iEbdIndex)) {
      ZbZdoPrivateData(maEdbTable[0].status) = gZdoNoMatch_c;
      ZbZdoPrivateData(maEdbTable[0].state) = mEdbStateWaitingForStatus_c;
      return 0;
    }
    ...
    {
      for(iEbdIndex=0; iEbdIndex < ZbStackTablesSizes(mEdbMaxEntries); iEbdIndex++)
      {
        pPtr = &ZbZdoPrivateData(maEdbTable[iEbdIndex]);
    ...
    if(ZbZdoPrivateData(gZdoInEndDeviceBind) == ZbStackTablesSizes(mEdbMaxEntries))
    {
      /* All the EndDeviceBind.Request were received */
      ZbZdoPrivateData(mEdbTicksLeft) = 0;
      ZbZdoPrivateData(mEdbProcessTicksLeft) = mEdbProcessTicksToExpire_c;
    }
  }
...
void ZdpEndDeviceBindGotResponse(zbNwkAddr_t aSrcAddr, zbStatus_t status)
{
  ...
  pPtr = &ZbZdoPrivateData(maEdbTable[iEdbIndex]);
  ...
  if(fSuccess)
  {
    ...
    pPtr = &ZbZdoPrivateData(maEdbTable[pPtr->iMatchIndex]);
...
index_t EdbFindEntry(zbNwkAddr_t aNwkAddr)
{
  index_t i;
  zdoEdbTable_t  *pPtr;
  for(i=0; i < ZbStackTablesSizes(mEdbMaxEntries); ++i)
  {
    pPtr = &ZbZdoPrivateData(maEdbTable[i]);
...
)
{
  ...
  if (!IsEqual2Bytes(ZbZdoPrivateData(maEdbTable[0].request.aProfileId), pPtr->request.aProfileId))
    return fFoundMatch;
  ...
  */
  if (Zdp_MatchClusters(pPtr->request.inputClusterList.cNumClusters,
      ZbZdoPrivateData(maEdbTable[0].request.outputClusterList.cNumClusters),
      pPtr->request.inputClusterList.pClusterList,
      ZbZdoPrivateData(maEdbTable[0].request.outputClusterList.pClusterList),
      TRUE))
  {
  ...
  /*
    ...
    entry. The entry 0 is the first device that sent the EndDeviceBindRequest.
  */
  if (Zdp_MatchClusters(ZbZdoPrivateData(maEdbTable[0].request.inputClusterList.cNumClusters),
      pPtr->request.outputClusterList.cNumClusters,
      ZbZdoPrivateData(maEdbTable[0].request.inputClusterList.pClusterList),
...
void EdbStartupBind(index_t iSrcEdbIndex, index_t iDstEdbIndex)
{
  zdoEdbTable_t  *pPtr = &ZbZdoPrivateData(maEdbTable[iSrcEdbIndex]);
  ...
  FLib_MemCpy(&pPtr->aMatchCluster[1], &ZbZdoPrivateData(gaZdpMatchedOutCluster),(sizeof(zbClusterId_t) * ZbZdoPrivateData(giMatchIndex)));

  /* The first entry has the amount of matches. */
  Set2Bytes(&pPtr->aMatchCluster[0], ZbZdoPrivateData(giMatchIndex));
  ZbZdoPrivateData(giMatchIndex) = 0x00;
...
bool_t EbdSendBindUnBind(bool_t fBind, index_t iEdbIndex)
{
  ...
  zdoEdbTable_t  *pPtr = &ZbZdoPrivateData(maEdbTable[iEdbIndex]);
  ...
  Copy8Bytes(bindRequest.destData.extendedMode.aDstAddress, ZbZdoPrivateData(maEdbTable[iOtherIndex].request.aSrcIeeeAddress));
  bindRequest.destData.extendedMode.dstEndPoint = ZbZdoPrivateData(maEdbTable[iOtherIndex].request.srcEndPoint);
...
void EdbFree(index_t iEdbIndex)
{
  ZbZdoPrivateData(maEdbTable[iEdbIndex].state) = 0;
}
...
index_t EdbAlloc(zbEndDeviceBindRequest_t *pReqFrame)
{
  ...
  for(iEdbIndex=0; iEdbIndex < ZbStackTablesSizes(mEdbMaxEntries); ++iEdbIndex)
  {
    /* free entry */
  ...
  if(iEdbIndex == ZbStackTablesSizes(mEdbMaxEntries))
...
bool_t EdbSendResponse(index_t i)
{
  ...
  ZbZdoPrivateData(gZdpResponseCounter) = pPtr->iTransactionId;
...
bool_t EdbSendTimeoutResponse(void)
{
  ...
  ZbZdoPrivateData(gZdoInEndDeviceBind) = FALSE;
