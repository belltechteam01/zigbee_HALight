...
#define SetMgmtNwkDiscReq()               (ZbZdoPrivateData(gMgmtNwkDiscReq) = TRUE)
#define SetstartIndexforNwkDisc(param)    ( ZbZdoPrivateData(gStartIndexforNwkDisc) = ( param ) )
#define GetStartIndexforNwkDisc()         ZbZdoPrivateData(gStartIndexforNwkDisc)
#define IncrementIndex( param )           ( param ) = ( ( ( param ) <  ZbStackTablesSizes(gMaxArray_c) )? ( param )++: 0 )
#define DecrementIndex( param )           ( param ) = ( ( ( param ) > 0 )? ( param )--: 0 )
#define StoreAddress( index, strAddress ) ( Copy2Bytes( ( ZbZdoPrivateData(strAddrClusterId[ ( index ) ].address )), ( strAddress ) ) )
#define GetClusterId( index )             (ZbZdoPrivateData(strAddrClusterId[(index)].clusterId))
#define SetClusterId(index, cluster)      ((ZbZdoPrivateData(strAddrClusterId[(index)].clusterId)) = (cluster))
#define GetAddress(index)                 (ZbZdoPrivateData(strAddrClusterId[(index)].address))
...
#define ZDP_SetSequenceId(index,sequenceId)              ( ( ZbZdoPrivateData(maEventProcessQueue[ ( index ) ].sequenceNumber )) = ( sequenceId ) )
#define ZDP_SetOriginalSequenceNumber(index,sequenceId)  ( ( ZbZdoPrivateData(maEventProcessQueue[ ( index ) ].sourceSeqNumber)) = ( sequenceId ) )
#define ZDP_SetOriginatorAddress(index,address)          ( Copy2Bytes( ( ZbZdoPrivateData(maEventProcessQueue[ ( index ) ].sourceAddress)) , ( address ) ) )
#define ZDP_GetOriginatorAddress(index)                  ( ZbZdoPrivateData(maEventProcessQueue[ ( index ) ].sourceAddress ))
#define ZDP_GetOriginalSequenceNumber(index)             ( ZbZdoPrivateData(maEventProcessQueue[ ( index ) ].sourceSeqNumber ))
#define ZDP_SetMachineId(index,MachineId)                ( ( ZbZdoPrivateData(maEventProcessQueue[ ( index ) ].machineId )) = ( MachineId ) )
#define ZDP_SetResponseSeqNum(index)                     ( ZbZdoPrivateData(gZdpResponseCounter )= ( index ) )
...
#define ZDP_GetServerMaskServices(serverMask)  ( *( (uint16_t *) ZbZdoPublicData(gaServerMask)) & *( (uint16_t *)( serverMask )) )
#define ZDP_GetServiceOn(service)              (*( (uint16_t *) ZbZdoPublicData(gaServerMask)) & (service))
...
*/
#define ZDP_GetIEEEAddressFormHoldingBindingInfoTable(index)  ( ZbZdoPrivateData(aDevicesHoldingItsOwnBindingInfo[ ( index ) ] ))
#endif
...
#define Zdp_UpdateBindResponseStatus(index, status)  (*((uint8_t *)&(ZbZdoPrivateData(maEventProcessQueue[(index)].data.bindingInfo))) = (status))
#endif
...
extern void FA_TimeoutHandler(zbTmrTimerID_t tmrId);
#endif
