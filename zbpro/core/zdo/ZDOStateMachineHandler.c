...
/* State Selected depending on device type (old state machine only) */
  ...
  #define mZDOCoordinatorState ZbZdoPrivateData(gZdoState)
  #define ZDOCoordinatorChangeState( state ) mZDOCoordinatorState = state
#elif ( gRouterCapability_d )
  #define mZDORouterState ZbZdoPrivateData(gZdoState)
  #define ZDORouterChangeState( state ) mZDORouterState = state
#else
  #define mZDOEndDeviceState ZbZdoPrivateData(gZdoState)
...
#define mcFormationRetries  ZbZdoPrivateData(mcDiscoveryRetries)
#define mcOrphanScanRetries ZbZdoPrivateData(mcDiscoveryRetries)
...
(
  tsEvent_t events  /*IN: events sent to ZDO State Machine */
)
{
...
{
  ZbZdoPrivateData(gZdoEvent) = event;
...
void ZDO_StartEx(ZdoStartMode_t startMode)
{
  ...
  /* start up the state machine */
   ZbZdoPublicData(gZdoStartMode) = startMode;

  /* needed for some of the other stack routes (global dependancy) */
  ZbZdoPrivateData(gZDOJoinMode) = ZDO_GetStartMode();
...
void ZDO_StopEx(ZdoStopMode_t stopMode)
{
  ...
  /* set stop mode and tell ZDO to stop network */
   ZbZdoPublicData(gZdoStopMode) = stopMode;
...
void ZDO_RetrieveProperCommissingSet(ZdoStartMode_t iStartupSet)
{
  ...
  */
  if(iStartupSet == gZdoStartMode_SasSet_c)
  {
    if(ASL_GetCommissioningSAS())
    {
      ...
      FLib_MemCpy(&ZbBeeStackGlobalsParams(gSAS_Ram), ASL_GetCommissioningSAS(), sizeof(zbCommissioningAttributes_t));
#if gStandardSecurity_d || gHighSecurity_d
  ...
  */
  ...
  if (iStartupSet == gZdoStartMode_RomSet_c)
  {
    ...
    FLib_MemCpy(&ZbBeeStackGlobalsParams(gSAS_Ram), (void *)(&gSAS_Rom), sizeof(zbCommissioningAttributes_t));
    /*
    ...
    */
    #else
    {
      ...
      FLib_MemCpy(&ZbBeeStackGlobalsParams(gSAS_Ram), (void *)(&gSAS_Rom[zbProInstance]), sizeof(zbCommissioningAttributes_t));
    }
    #endif
  }
...
void RestoreIncomingFrameCounter(void)
{
    ...
    ZbBeeStackNwkGlobals(gNwkData.aNwkSecurityMaterialSet[0].pIncomingFrameCounterSet) = ZbBeeStackNwkGlobals(gaIncomingFrameCounterSet1);
    ZbBeeStackNwkGlobals(gNwkData.aNwkSecurityMaterialSet[1].pIncomingFrameCounterSet) = ZbBeeStackNwkGlobals(gaIncomingFrameCounterSet2);
}
#endif
    ...
    if (!ZDO_ValidateNwkData())
    {
      ...
      if (!Cmp8BytesToZero(ApsmeGetRequest(gApsUseExtendedPANID_c)))
      {
        ...
        */
        ...
        ZbZdoPrivateData(gZDOJoinMode) = gZdoStartMode_NwkRejoin_c;         
        ZDO_SetStartMode(gZdoStartMode_NwkRejoin_c);
      }
    }
  }
...
bool_t ZDO_IsValidParentInfo(void)
{
  neighborTable_t *pNTE = ZbBeeStackNwkGlobals(gaNeighborTable);
...
#if gStandardSecurity_d || gHighSecurity_d
          && (pSecurityMaterialSet)
          && (!Cmp16BytesToZero(pSecurityMaterialSet->nwkKey))
#endif
  );
  /*
...
void ZDO_StateMachineInitialState(void)
{
...
#endif
  uint8_t panIdConflictDetectstatus = FALSE;
  ...
  /* set PibMac2003Compatibility */
  {
    uint8_t macPibCapability = TRUE;
    SetPibAttributeValue( gMPibMac2003Compatibility_c, &macPibCapability);
  }
  /* turn off MAC feature of PAN ID conflict detection (always) */
  (void)SetPibAttributeValue( gMPibPanIdConflictDetection_c, &panIdConflictDetectstatus );
  ...
  /* Set the Aps designated coordinator value for combo devices. */
#if gComboDeviceCapability_d
  if (NlmeGetRequest(gDevType_c) == gCoordinator_c)
  ...
  ZbZdoPrivateData(gZDO_SecureRejoin) = ApsmeGetRequest(gApsUseInsecureJoin_c);
  ...
  */
...
#if gComboDeviceCapability_d && (!gASL_EnableZllTouchlinkCommissioning_d)
  if (NlmeGetRequest(gDevType_c) == gCoordinator_c)
#endif
  {
    ZdoSetNwkManagerOn( ZbZdoPublicData(gaServerMask));
  }
#else
  ZdoSetNwkManagerOff( ZbZdoPublicData(gaServerMask));
#endif
...
#endif
  {
    /* ZED: remember original polling rate */
     ZbZdoPublicData(gZdoOrgPollRate) = NlmeGetRequest(gNwkIndirectPollRate_c);
  ...
  {
    /* 053474r20 ZigBee Pro spec Section 2.5.5.5.6.2 Startup Procedure */
    if (!Cmp8BytesToZero(NlmeGetRequest(gNwkExtendedPanId_c)) && !ApsmeGetRequest(gApsDesignatedCoordinator_c))
    {
      ...
      ZbZdoPrivateData(gZDOJoinMode) = gZdoStartMode_FindAndRejoin_c;
    }
  ...
  */
  ...
  if(!ZDO_IsValidStartMode(ZbZdoPrivateData(gZDOJoinMode)))
  {
   /* force to associate mode if invalid PAN, extended PAN or short address (or NWK not started) */
    ZbZdoPrivateData(gZDOJoinMode) = gZdoStartMode_Associate_c;
    ZDO_SetStartMode(gZdoStartMode_Associate_c);
    /*
      There is no need to check if the device is on the network or not, the validation
      happens on the initial state of the zdo state machine and this state only takes control
  ...
  */
  if((ZbZdoPrivateData(gZDOJoinMode) != gZdoStartMode_Associate_c) && (ZDO_GetStartupSet() == gZdoStartMode_NvmSet_c))
  {
    /* Any start mode other than association assumes that the device was on the network before. */
    ZDO_SecUpdateSecureCounters();
    ZdoNwkMng_SaveToNvm(zdoNvmObject_SecureSet_c, NULL);
  }
#if gApsLinkKeySecurity_d
  /* Clear the APS security counters for unsecure rejoin */
  if((ZDO_GetStartMode() != gZdoStartMode_FindAndRejoin_c)
    && (ZDO_GetStartMode() != gZdoStartMode_SilentStart_c)
      && (ApsmeGetRequest(gApsUseInsecureJoin_c)))
  {
	  ZDO_SecClearAPSIncomingCounters();
  }
#endif
#endif

  /* move on to discovery state (all device types if associating) */
  if(ZbZdoPrivateData(gZDOJoinMode) == gZdoStartMode_Associate_c)
  {
  ...
  /* silent starting */
  else if(ZbZdoPrivateData(gZDOJoinMode) == gZdoStartMode_SilentStart_c)
  {
    if(NlmeGetRequest(gDevType_c) == gCoordinator_c)
    {
  ...
  else if (ZbZdoPrivateData(gZDOJoinMode) == gZdoStartMode_NwkRejoin_c)
  {
  ...
  else if (ZbZdoPrivateData(gZDOJoinMode) == gZdoStartMode_FindAndRejoin_c)
  {
    /* Bring all possible data back, and do the rejoin, the node need to discover netwroks. */
    ZDO_SetState(gZdoDiscoveringNetworksState_c);
    ZDO_SendEvent(gZdoEvent_StartingDiscovery_c);
...
void ZDO_StateMachineDiscoveringNetworks(void)
{
  ...
  if(ZDO_GetEvent() == gZdoEvent_StartingDiscovery_c)
  {
...
#endif
    {
      ...
      ZbZdoPrivateData(mcDiscoveryRetries) = ZbBeeStackGlobalsParams(gBeeStackConfig.gNwkFormationAttempts);
      iTimeout = ZbBeeStackGlobalsParams(gBeeStackConfig.gFormationAttemptsTimeOut);
...
#if (!gCoordinatorCapability_d )
    {
      ...
      ZbZdoPrivateData(mcDiscoveryRetries) = ZbBeeStackGlobalsParams(gBeeStackConfig.gNwkDiscoveryAttempts);
      iTimeout = ZbBeeStackGlobalsParams(gBeeStackConfig.gDiscoveryAttemptsTimeOut);
    ...
    if(iTimeout)
    {
      ...
      ZbTMR_StartSecondTimer( gZdoDiscoveryAttemptTimerID, iTimeout, ZDO_StateMachineTimerCallBack );
    }

    /* prepare for discovery */
    ZbZdoPrivateData(mDiscoveryFirstTime) = TRUE;
    ZbZdoPrivateData(mcScanRetries) = NlmeGetRequest(gNwkScanAttempts_c);
  }
  ...
  else if(ZDO_GetEvent() == gZdoEvent_DiscoveryFailure_c || ZDO_GetEvent() == gZdoEvent_DiscoverySuccess_c)
  {
    ...
    */
    ZbZdoPrivateData(mcScanRetries)--;    
    if (ZbZdoPrivateData(mcScanRetries) == 0)
    {
#if gCoordinatorCapability_d || gComboDeviceCapability_d
#if gComboDeviceCapability_d
      if (NlmeGetRequest(gDevType_c) == gCoordinator_c)
      ...
      ZbTMR_StopTimer(gZdoDiscoveryAttemptTimerID);
      return;
    }

    /* start the between scans timer when done will generate the gZdoEvent_NextDiscovery_c event */
    ZbTMR_StartSingleShotTimer(gTimeBetScansTimerID,
  ...
  else if( ZDO_GetEvent() == gZdoEvent_NextDiscovery_c )
  {
    ...
    if( ZbZdoPrivateData(mcDiscoveryRetries) )
    {
      /* don't decrement the first time, as we don't want to stop if only set to 1 attempt */
      if(ZbZdoPrivateData(mDiscoveryFirstTime)) 
      {        
        ZbZdoPrivateData(mDiscoveryFirstTime) = FALSE;
      }
      else 
      { /*only decrement the nwk discovery counter, if scan attempts are done.*/
        if (ZbZdoPrivateData(mcScanRetries) == NlmeGetRequest(gNwkScanAttempts_c)) 
        {        
          ZbZdoPrivateData(mcDiscoveryRetries)--;      
          if( !ZbZdoPrivateData(mcDiscoveryRetries) )
          {
...
void ZDO_StateStartRouterCheck(void)
{
  switch(ZDO_GetEvent())
  {
...
#endif
      { 
        bool_t pibValue = TRUE;
        (void)SetPibAttributeValue(gMPibBeaconResponseDenied_c, &pibValue);
    ...
    case gZdoEvent_JoinFailure_c:
    	Zdo_GenerateEventZdo2App(gZDOToAppMgmtJoinNwkFailed_c);
    	/* Fall through */
    ...
    case gZdoEvent_StartRouterFailure_c:
      ...
      */
      if (ZbZdoPrivateData(mcDiscoveryRetries) == 1) {
      ...
      /* start up the in-between timer */
        ZbTMR_StartTimer(gZdoFormationAttemptTimerID, gTmrLowPowerSingleShotMillisTimer_c, ZbTmrMilliseconds(NlmeGetRequest(gRejoinInterval_c) * 1000), ZDO_StateMachineTimerCallBack);
      }
    break;
...
void ZDO_StateMachineJoining(void)
{
  switch(ZDO_GetEvent())
  {
    ...
    case gZdoEvent_Joining_c:
      ...
      ZDO_CheckForNwkRejoin(ZbZdoPrivateData(gZDOJoinMode));

      /* attempt join the network (join mode may be any of the 5 join modes) */
      ZdoNwkMng_GenerateNwkJoinRequest( ZbZdoPrivateData(gZDOJoinMode ));
    break;
    ...
    case gZdoEvent_JoinSuccess_c:
#if (gStandardSecurity_d || gHighSecurity_d)
      /* Always go through authentication state. */
      ZDO_SetState(gZdoDeviceAuthenticationState_c);
      ZDO_SendEvent(gZdoEvent_AuthenticateStart_c);
      ApsmeSetRequest(gApsUseInsecureJoin_c, ZbZdoPrivateData(gZDO_SecureRejoin));
#else
#if (gRouterCapability_d || gComboDeviceCapability_d)
#if gComboDeviceCapability_d
      if(NlmeGetRequest(gDevType_c) == gRouter_c)
#endif
      {
        /* Move to Running State through Start Router Confirm */
        ZdoNwkMng_GenerateStartRouterReq();
      }
#endif //(gRouterCapability_d || gComboDeviceCapability_d)
#if (gEndDevCapability_d || gComboDeviceCapability_d)
#if gComboDeviceCapability_d
    if(NlmeGetRequest(gDevType_c) == gEndDevice_c)
#endif
...
void ZDO_StateMachineAuthenticatingDevice(void)
{
  switch(ZDO_GetEvent())
  {
    case gZdoEvent_AuthenticateStart_c:
      ...
      if (((ZbZdoPrivateData(gZDOJoinMode) != gAssociationJoin_c) && (ZbZdoPrivateData(gZDOJoinMode) != gNwkRejoin_c)) ||
          ((ZbZdoPrivateData(gZDOJoinMode) == gNwkRejoin_c) && !ApsmeGetRequest(gApsUseInsecureJoin_c)))
      {
...
#endif
      {
        if (!IsLocalDeviceReceiverOnWhenIdle())
        {
          ...
          */
          if ( ZbZdoPublicData(gZdoOrgPollRate) > NlmeGetRequest(gAuthenticationPollTimeOut_c))
          {
      ...
      */
      ZbTMR_StartSingleShotTimer( gZdoEstablishKeysTimerID,
                                ZbBeeStackGlobalsParams(gBeeStackConfig.gAuthenticationTimeOut),
                                ZDO_StateMachineTimerCallBack );
    ...
    case gZdoEvent_AuthenticationSuccess_c:
      /* Stop the timers */
      ZbTMR_StopTimer( gZdoEstablishKeysTimerID );
#if ( gRouterCapability_d || gCoordinatorCapability_d ) || (gComboDeviceCapability_d)
#if gComboDeviceCapability_d
      if ( NlmeGetRequest( gDevType_c ) != gEndDevice_c )
#endif
...
#endif
      {
        NWK_MNGTSync_ChangePollRate( ZbZdoPublicData(gZdoOrgPollRate));
      }
#endif
...
void ZDO_StateMachineRunningState(void)
{
  ...
  */
...
#endif
  {
    ...
    {
#if (gComboDeviceCapability_d)
      ZbBeeStackNwkGlobals(gLpmIncluded) = TRUE;
#endif
      ZDO_AllowDeviceToSleep();
      (void)ZDO_NLME_ChangePollRate( ZbZdoPublicData(gZdoOrgPollRate));
    }
  ...
  ZDO_SetState(ZbZdoPrivateData(maZdoRunning[NlmeGetRequest(gDevType_c)]));
...
#endif
  {
    ...
    */
    if(NlmeGetRequest(gNwkIsConcentrator_c))
    {
      ...
      /*
        ZigBee spec. 053474r19
        The time in seconds between concentrator route discoveries. If set to
        0x0000, the discoveries are done at start up and by the next higher
        layer only.
      */
      ZDO_SendRouteDiscoveryManyToOne(NlmeGetRequest(gNwkConcentratorRadius_c), !ZbBeeStackNwkGlobals(gNwkHighRamConcentrator));
    }
  }
#endif
  ...
  */
...
#if gRouterCapability_d || gComboDeviceCapability_d
    if ((NlmeGetRequest(gDevType_c) == gRouter_c) && !ZbBeeStackNwkGlobals(gActAsZed))
    {
  ...
  ZbZdoPrivateData(gZdoEvent) = gZdoEvent_Unused_c;
  ...
  */
  Zdo_GenerateEventZdo2App( ZbZdoPrivateData(maZdo2AppEvents[NlmeGetRequest(gDevType_c)] ));
}
...
void ZDO_StateMachineLeaving(void)
{
  ...
  else if (ZDO_GetEvent() == gZdoEvent_LeaveNetwork_c )
  {
    ...
    */
    ZbTMR_StartSingleShotTimer( gZdoLeavingTimerID,
...
void ZDO_StateMachineStopping(void)
{
  ...
  /*
    ...
    then stop.
  */
  if ( ZbZdoPublicData(gZdoStopMode) & gZdoStopMode_Announce_c)
  {
    /*
      Clearing the bit is important to avoid looping in the stop state doing the same
      action over and over again.
    */
     ZbZdoPublicData(gZdoStopMode) &= ~gZdoStopMode_Announce_c;
  ...
  fFullReset =  ZbZdoPublicData(gZdoStopMode) & gZdoStopMode_ResetTables_c;
...
/* WARNING WARNING WARNING WARNING WARNING WARNING WARNING WARNING WARNING WARNING WARNING
  ...
  /* save NVM */
  if( ZbZdoPublicData(gZdoStopMode) & gZdoStopMode_ResetNvm_c)
  {
    ...
    /*
      ...
      action over and over again.
    */
     ZbZdoPublicData(gZdoStopMode) &= ~gZdoStopMode_ResetNvm_c;
  }

  /* restart if requested */
  if( ZbZdoPublicData(gZdoStopMode) & gZdoStopMode_StopAndRestart_c)
  {
     ZbZdoPublicData(gZdoStopMode) &= ~gZdoStopMode_StopAndRestart_c;
    /*
      Clearing the bit is important to avoid looping in the stop state doing the same
      action over and over again.
    */
...
void ZDO_StateMachineTimerCallBack(uint8_t timerId)
{
  uint8_t leavingTimerId = gZdoLeavingTimerID;
  ...
  if((timerId == gZdoFormationAttemptTimerID) || timerId == leavingTimerId)
  {
...
{
  if ((NlmeGetRequest(gDevType_c) != gEndDevice_c) && !ZbBeeStackNwkGlobals(gActAsZed))
  {
    return;
  }
#if gEndDevCapability_d || gComboDeviceCapability_d
...
/* A polling ZED can not be a server of any kind. */
  ...
  if (NlmeGetRequest(gDevType_c) == gEndDevice_c)
  {
    ...
    if (!IsLocalDeviceReceiverOnWhenIdle())
    {
      ...
      BeeUtilZeroMemory( ZbZdoPublicData(gaServerMask), sizeof( ZbZdoPublicData(gaServerMask)));
    }
  }
#endif
}
...
void ZDO_ApsReset(bool_t fFullReset, apsmeMessage_t *pApsmeMsg)
{
  ...
  if (fFullReset)
  {
    ...
    /*
      ...
      action over and over again.
    */
     ZbZdoPublicData(gZdoStopMode) &=  ~gZdoStopMode_ResetTables_c;
  }
...
/************************************************************************************
* Send a Many-To-One Nlme-Route-Discovery.req to all routers 0xFFFC.
* This function only require the 
...
************************************************************************************/
#if gConcentratorFlag_d
void ZDO_SendRouteDiscoveryManyToOne
(
  uint8_t iRadius,
