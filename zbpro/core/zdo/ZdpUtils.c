...
#define Zdp_CacheTableEntryAddr(index)  (&ZbZdoPrivateData(gaDiscoveryStoreTable[(index)]))
#define Zdp_CacheTableEntrySize(index)  (sizeof(ZbZdoPrivateData(gaDiscoveryStoreTable[(index)])))
/**********************************************************************
***********************************************************************
* Public Memory
***********************************************************************
  ...
  if (requestType != mSingleDevResp && requestType != mExtendedDevResp)
  {
    ...
    if (IsSelfIeeeAddress(pMessageComingIn) || ((clusterId == gIEEE_addr_req_c) && (IsSelfNwkAddress(pMessageComingIn))))
    {
      ...
      else if( requestType == mExtendedDevResp )  /*Request Type*/
      {
        ...
        for( ; cIndex < ZbStackTablesSizes(gMaxNeighborTableEntries); cIndex++ )
        {
    ...
    {
#if gDiscovery_store_rsp_d || gBkup_Discovery_cache_d
      if (ZdoGetPrimaryDiscoveryCacheBitMask( ZbZdoPublicData(gaServerMask)) || ZdoGetBackupDiscoveryCacheBitMask( ZbZdoPublicData(gaServerMask)))
      {
      ...
      for(cIndex = 0 ; cIndex < ZbStackTablesSizes(gMaxNeighborTableEntries); cIndex++ )
      {
        pNeighborTable=NlmeGetRequestTableEntry(gNwkNeighborTable_c, cIndex);
        /* Once we get the info from the neighbor table we need to find out how is our child */
      ...
      if (!ZbZdoPrivateData(gIsABroadcast)) 
      {
...
)
{
  ...
  {
    if (ZbBeeStackGlobalsParams(gComplexDesc.fieldCount))
    {
      pMessageGoingOut->status = gZbSuccess_c;
      pMessageGoingOut->length = sizeof(zbComplexDescriptor_t);
      FLib_MemCpy( &pMessageGoingOut->complexDescrip,&ZbBeeStackGlobalsParams(gComplexDesc),sizeof(zbComplexDescriptor_t));
...
#if gComplex_Desc_rsp_d
        /* Set the Complex descriptor available bit */
        pNodeDescriptor->logicalType |= (1<<3);
#endif
#if gUser_Desc_rsp_d
        pNodeDescriptor->logicalType |= (1<<4);
#endif

        /* -> Frequency band used by the radio. */
        pNodeDescriptor->apsFlagsAndFreqBand = ZbBeeStackGlobalsParams(gBeeStackConfig.apsFlagsAndFreqBand);

        /* -> MAC capability Flag. */
        pNodeDescriptor->macCapFlags = NlmeGetRequest(gNwkCapabilityInformation_c);

        /* -> Manufacturer Code Flag. */
        Copy2Bytes(pNodeDescriptor->aManfCodeFlags,ZbBeeStackGlobalsParams(gBeeStackConfig.manfCodeFlags));

        /* -> Maximum buffer Size. This is the worst case scenario. */
        pNodeDescriptor->maxBufferSize = GetMaxNsduSize();

        /* -> The server Mask to be sended in the request payload. */
        Copy2Bytes(pNodeDescriptor->aServerMask, ZbZdoPublicData(gaServerMask));

        /* -> Default Value of Maximum Outgoing Transfer Size. Worst case */
	Copy2Bytes(pNodeDescriptor->aMaxOutTransferSize, ZbBeeStackGlobalsParams(gBeeStackConfig.aMaxOutTransferSize));
        Copy2Bytes(pNodeDescriptor->aMaxTransferSize,ZbBeeStackGlobalsParams(gBeeStackConfig.maxTransferSize));

        /* -> The descriptor capability field. */
        pNodeDescriptor->descCapField = ZbBeeStackGlobalsParams(gBeeStackConfig.descCapField);  
#if gExtended_Active_EP_req_d
        pNodeDescriptor->descCapField |= (1<<0);
#endif
#if gExtended_Simple_Desc_rsp_d
        pNodeDescriptor->descCapField |= (1<<1);
#endif
      ...
      else
      {
        ...
        pPtr = &ZbZdoPrivateData(gaDiscoveryStoreTable[cIndex]);
...
/* iar warning of statement unreachable */
    ...
    case gPower_Desc_req_c:
      if (IsSelfNwkAddress(((zbPowerDescriptorRequest_t *)pMessageComingIn)->aNwkAddrOfInterest))
      {
        ...
        Copy2Bytes(pData, &ZbBeeStackGlobalsParams(gBeeStackConfig.currModeAndAvailSources));
...
/* iar warning of statement unreachable */
    ...
    case  gUser_Desc_req_c:
      ...
      {
        pData[0] = ZbBeeStackGlobalsParams(gUserDesc.length);
        FLib_MemCpy(pData + sizeof(ZbBeeStackGlobalsParams(gUserDesc.length)), ZbBeeStackGlobalsParams(gUserDesc.aUserDescription), sizeof(ZbBeeStackGlobalsParams(gUserDesc.aUserDescription)));
...
)
{
  ...
  if (!ZdoGetPrimaryDiscoveryCacheBitMask( ZbZdoPublicData(gaServerMask)))
  {
...
)
{
  ...
  */
  ...
  /*
    Store in cache if the binding src is not us or if it's us and we stored
    it successfully in the APS.
  */
  if ((bindUnbindStatus == gZdoInvalidRequestType_c) ||  
  ...
  */
  if (!ZdoGetPrimaryBindingTableCacheBitMask( ZbZdoPublicData(gaServerMask)))
  {
    if(bindUnbindStatus == gZdoInvalidRequestType_c)
...
)
{
  ...
  limit = (tableType)? gMaximumBindingCacheTableList_c : ZbStackTablesSizes(gApsMaxBindingEntries);
...
)
{
  ...
  if (tableType)
    limit = ZbStackTablesSizes(gMaximumBindingCacheTableList);
  else
    limit = ZbStackTablesSizes(gApsMaxBindingEntries);

  for (i = requestIndex; i < limit; i++)
  {
    if (tableType)
    {
      if (ZdpBindIsFree(ZbZdoPrivateData(gaBindingCacheTable[i])))
        continue;
    }
    else
    {
...
)
{
  ...
  uint8_t totalBindEntries = gApsMaxClusterList * ZbStackTablesSizes(gApsMaxBindingEntries);
  ...
  */
  ...
  /*
    ...
    requested index.
  */
  for (i = 0; i < ZbStackTablesSizes(gApsMaxBindingEntries); i++)
  {
    ...
    */
    if ((i >= iBindIndex) && (iNumOfEntries))
    {
      ...
      /* Moving to the next Binding Table entry that contains a new set of clusters, so start from index 0. */
      iClusterIndex = 0;
    }
  }
...
)
{
  ...
  if ((startIndex > ZbStackTablesSizes(gMaximumBindingCacheTableList)) || (index == gZdoNoEntry_c))
  {
    /* Even if there is no entry to return, the status must be success (2.4.4.3.4 r17) */
    return (0);
  }
  ...
  for (i = 0; i< ZbStackTablesSizes(gMaximumBindingCacheTableList); i++)
  {
    pPtr = (void *)&ZbZdoPrivateData(gaBindingCacheTable[i]);
...
)
{
  ...
  for (cIndex = index; cIndex < ZbStackTablesSizes(gMaximumBindingCacheTableList); cIndex++)
  {
    pPtr = &ZbZdoPrivateData(gaBindingCacheTable[cIndex]);
...
)
{
  ...
  BeeUtilZeroMemory(&ZbZdoPrivateData(gaBindingCacheTable[cIndex]), sizeof(ZbZdoPrivateData(gaBindingCacheTable[cIndex])));
  ZbZdoPrivateData(gBindingCacheTableCounter)--;
  return gZbSuccess_c;
}
#endif
...
{
  index_t  cIndex;
  for (cIndex = 0; cIndex < ZbStackTablesSizes(gMaximumBindingCacheTableList); cIndex++)
  {
    if (addressMode == zbGroupMode)
    {
      if (FLib_MemCmp(&ZbZdoPrivateData(gaBindingCacheTable[cIndex]), pBindEntry, MbrOfs(zbApsmeBindEntry_t, aDstAddr[0])+ sizeof(zbGroupId_t)))
        return cIndex;
    }
    else if (addressMode == zbIEEEMode)
    {
      if (FLib_MemCmp(&ZbZdoPrivateData(gaBindingCacheTable[cIndex]), pBindEntry, sizeof(zbApsmeBindEntry_t)))
        return cIndex;
    }
  }
  return gZdoNoEntry_c;
...
)
{
  ...
  if (ZbZdoPrivateData(gDeviceHoldingItsOwnBindingInfo_Counter) >= ZbStackTablesSizes(gMaximumDevicesHoldingBindingInfo))
    return gZdoTableFull_c;

/*Search into the entire  Table */
  for (cIndex = 0; cIndex< ZbStackTablesSizes(gMaximumDevicesHoldingBindingInfo); cIndex++)
  {
    if(IsEqual8Bytes(ZbZdoPrivateData(aDevicesHoldingItsOwnBindingInfo[cIndex]), aIeeeAddress))
      return gZbSuccess_c;
    if (Cmp8BytesToZero(ZbZdoPrivateData(aDevicesHoldingItsOwnBindingInfo[cIndex])) && (zeroIndex == 0xff))
      zeroIndex = cIndex;
  }
  ...
  if (zeroIndex != 0xff)
  {
    Copy8Bytes(ZbZdoPrivateData(aDevicesHoldingItsOwnBindingInfo[zeroIndex]), aIeeeAddress);
    ZbZdoPrivateData(gDeviceHoldingItsOwnBindingInfo_Counter)++;
      return gZbSuccess_c;
  }
  else
    return gZdoTableFull_c;
...
#if gBind_Register_rsp_d || gRecover_Bind_Table_rsp_d
/*-------------------- ZDP_GetBindingCacheEntryListByAddressAndStartIndex --------------------
  This function is ZDP internal.
  ...
  IN: The binding table index from where to start looking for entries. 
...
*/
zbuCounter16_t ZDP_GetBindingCacheEntryListByAddressAndStartIndex
(
  zbIeeeAddr_t  aIeeeAddress,   /* IN: The IEEE Addres to lok inthe binding table, a broadcast address means all entries. */
        uint16_t       startIndex,    /* IN: The binding table index from where to start looking for entries. */  
...
)
{
  ...
  for (cIndex = startIndex; cIndex < ZbStackTablesSizes(gMaximumBindingCacheTableList); cIndex++)
  {
    pPtr = &ZbZdoPrivateData(gaBindingCacheTable[cIndex]);
    if (ZdpBindIsFree(ZbZdoPrivateData(gaBindingCacheTable[cIndex])))
      continue;
...
)
{
  ...
  for (cIndex = 0;cIndex < ZbStackTablesSizes(gMaximumDevicesHoldingBindingInfo); cIndex++)
    if (IsEqual8Bytes(ZbZdoPrivateData(aDevicesHoldingItsOwnBindingInfo[cIndex]), aAddress))
    return (cIndex);
...
)
{
  ...
  if (ZbZdoPrivateData(gBindingCacheTableCounter) >= ZbStackTablesSizes(gMaximumBindingCacheTableList))
    return FALSE;
...
)
{
  switch(machineId)
  {
    ...
    case gReplace_Device_req_c: 
      Process_ReplaceDevices(sequenceNumber);
      break;
    case gRemove_Bkup_Bind_Entry_req_c: 
      Process_RemoveDevReq(sequenceNumber);
      break;
#endif
      
#if (gBind_rsp_d || gUnbind_rsp_d) && gBindCapability_d
    case gBind_req_c: 
      Process_BindReq(sequenceNumber);
      break;
#endif
      
#if gBind_Register_rsp_d
    case gInformDevices_c: 
      Process_SourceBindDevices(sequenceNumber);
      break;
#endif
    default: 
      return;
...
)
{
  ...
  for (cIndex = 0; cIndex < gMaximumEventsToProcess_c; cIndex++)
  {
    pPtr = &ZbZdoPrivateData(maEventProcessQueue[cIndex]);
...
)
{
  ...
  ZDP_SetOriginalSequenceNumber(cIndex, ZbZdoPrivateData(gZdpResponseCounter));
  ...
  switch(machineId)
  {
    ...
    */
    ...
    case gInformDevices_c:
      ...
      ZDP_StateMachine(machineId,ZbZdoPrivateData(gZdpSequence));
    break;
    case gReplace_Device_req_c:
    case gRemove_Bkup_Bind_Entry_req_c:
      /*
...
{
  zbSystemServerDiscoveryStore_t  *pPtr = &ZbZdoPrivateData(mSystemServerDiscResponse[3]);
  switch (service)
  {
...
)
{
  ...
  */
  ...
  i = ZbZdoPrivateData(maEventProcessQueue[cIndex].data.replaceInfo.index);
  pPtr = &ZbZdoPrivateData(gaBindingCacheTable[i]);
...
)
{
  ...
  */
  ...
  FLib_MemCpy(pMessageGoingOut, 
              &ZbZdoPrivateData(gaBindingCacheTable[i]),sizeof(zbStoreBkupBindEntryRequest_t));
  GetAddressFromService(ZdoBackupBindingTableCache_c, aNwkAddress);
...
void ReplaceData(index_t cIndex, zbAddrMode_t addressMode)
{
  ...
  i = ZbZdoPrivateData(maEventProcessQueue[cIndex].data.replaceInfo.index);
  pBind = &ZbZdoPrivateData(gaBindingCacheTable[i]);
  pEvent = &ZbZdoPrivateData(maEventProcessQueue[cIndex]);
...
)
{
  ...
  *((uint8_t *)pMessageGoingOut + SapHandlerDataStructureOffSet) = status;
  Zdp_GenerateDataReq(gReplace_Device_rsp_c, ZbZdoPrivateData(maEventProcessQueue[cIndex].sourceAddress),(afToApsdeMessage_t *)pMessageGoingOut, MbrSizeof(zbReplaceDeviceResponse_t, status));
}
...
)
{
  ...
  if (ZbZdoPrivateData(mDiscoveryStoreTableEntrys) < ZbStackTablesSizes(gMaximumDiscoveryStoreTableSize))
  {
    for (cIndex = 0; cIndex < ZbStackTablesSizes(gMaximumDiscoveryStoreTableSize); cIndex++)
    {
      pPtr = Zdp_CacheTableEntryAddr(cIndex);
          ...
          Cmp8BytesToZero(Zdp_CacheTableIeeeAddr(pPtr)))
      {
        Copy2Bytes(Zdp_CacheTableNwkAddr(pPtr), aNwkAddress);
        Copy8Bytes(Zdp_CacheTableIeeeAddr(pPtr), aIeeeAddress);
        ZbZdoPrivateData(mDiscoveryStoreTableEntrys)++;
        return gZbSuccess_c;
      }
    }
  }
...
)
{
  ...
  switch(serverMaskToMach)
  {
    case ZdoPrimaryTrustCenter_c:
      if ((Cmp2BytesToZero(ZbZdoPrivateData(mSystemServerDiscResponse[0].aServerMask))) ||
          (!IsEqual2Bytes(aNwkAddress, ZbZdoPrivateData(mSystemServerDiscResponse[0].aNwkAddress))))
      {
        return FALSE;
      }
    break;
    case ZdoBackupTrustCenter_c:
      if ((Cmp2BytesToZero(ZbZdoPrivateData(mSystemServerDiscResponse[1].aServerMask))) ||
          (!IsEqual2Bytes(aNwkAddress, ZbZdoPrivateData(mSystemServerDiscResponse[1].aNwkAddress))))
      {
        return FALSE;
      }
    break;
    case ZdoPrimaryBindingTableCache_c:
      if ((Cmp2BytesToZero(ZbZdoPrivateData(mSystemServerDiscResponse[2].aServerMask))) ||
          (!IsEqual2Bytes(aNwkAddress, ZbZdoPrivateData(mSystemServerDiscResponse[2].aNwkAddress))))
      {
        return FALSE;
      }
    break;
    case ZdoBackupBindingTableCache_c:
      if ((Cmp2BytesToZero(ZbZdoPrivateData(mSystemServerDiscResponse[3].aServerMask))) ||
          (!IsEqual2Bytes(aNwkAddress, ZbZdoPrivateData(mSystemServerDiscResponse[3].aNwkAddress))))
      {
        return FALSE;
      }
    break;
    case ZdoPrimaryDiscoveryCache_c:
      if ((Cmp2BytesToZero(ZbZdoPrivateData(mSystemServerDiscResponse[4].aServerMask))) ||
          (!IsEqual2Bytes(aNwkAddress, ZbZdoPrivateData(mSystemServerDiscResponse[4].aNwkAddress))))
      {
        return FALSE;
      }
    break;
    case ZdoBackupDiscoveryCache_c:
      if ((Cmp2BytesToZero(ZbZdoPrivateData(mSystemServerDiscResponse[5].aServerMask))) ||
          (!IsEqual2Bytes(aNwkAddress, ZbZdoPrivateData(mSystemServerDiscResponse[5].aNwkAddress))))
      {
        return FALSE;
      }
    break;
...
)
{
  ...
  Zdp_GenerateDataReq(GetClusterId(ZbZdoPrivateData(gIndex)) | ResponseClusterId, GetAddress(ZbZdoPrivateData(gIndex)), (afToApsdeMessage_t *)pMsg, dataLength);
  ZbZdoPrivateData(gIndex) = ( ( ( ZbZdoPrivateData(gIndex )) <  ZbStackTablesSizes(gMaxArray) )? ( ZbZdoPrivateData(gIndex ))++: 0 );
}
#endif
...
)
{
  ...
  */
  ...
  /* If request was a broadcast and status is not success don't respond*/
  if (ZbZdoPrivateData(gIsABroadcast) && (status != gZbSuccess_c))
    return;
  ...
  /* Send the response */
  Zdp_GenerateDataReq(gMgmt_Leave_rsp_c, ZbZdoPrivateData(mgmtLeaveResponseInfo), pMsg, sizeof(zbMgmtLeaveResponse_t));
}
#endif
...
)
{
  ...
  if (IsBroadcastAddress(ZbZdoPrivateData(aSrcAddress)))
    return;
  ...
  if (!pMsg)
  {
    Copy2Bytes(ZbZdoPrivateData(aSrcAddress), gaBroadcastAddress);
    return;
  }
  ...
  *((uint8_t *)pMsg + ApsmeGetAsduOffset()) = iStatus;
  ...
  Zdp_GenerateDataReq(gMgmt_Direct_Join_rsp_c, ZbZdoPrivateData(aSrcAddress), pMsg, sizeof(zbMgmtDirectJoinResponse_t));
  Copy2Bytes(ZbZdoPrivateData(aSrcAddress), gaBroadcastAddress);
}
#endif
  ...
  for(iOutCluster=0; iOutCluster < cOutClusters; iOutCluster++)
  {
    for(iInCluster=0; iInCluster < cInClusters; iInCluster++)
    {
      if(IsEqual2Bytes(aInList[iInCluster], aOutList[iOutCluster]))
      {
        ...
        {
          /* Used for binding... */
          index_t matchIndex = ZbZdoPrivateData(giMatchIndex);
          Copy2Bytes(ZbZdoPrivateData(gaZdpMatchedOutCluster[matchIndex]),aOutList[iOutCluster]);
          ZbZdoPrivateData(giMatchIndex)++;
        }
        /* found match */
        status = TRUE;
      }
...
)
{
  ...
  if (ZbZdoPrivateData(mSystemServerDiscoveryResponsesCounter) >= ZbStackTablesSizes(gMaximumSystemServerciesResponses))
    return ;
...
#endif  
  for (cIndex=0; cIndex < ZbStackTablesSizes(gMaximumSystemServerciesResponses); cIndex++)
  {
    pPtr = &ZbZdoPrivateData(mSystemServerDiscResponse[cIndex]);
    ...
    if (Cmp2BytesToZero(pPtr->aServerMask) &&(TwoBytesToUint16(aServerMask) & bitMask))
    {
      ...
      ZbZdoPrivateData(mSystemServerDiscoveryResponsesCounter)++;
    }
...
//replaced ASL_Device_annce
...
#endif
  zbEndDeviceAnnounce_t  *pkgPayload;
  
  /* Memory is freed by the end application */
  afToApsdeMessage_t  *pMsgIn = AF_MsgAlloc();
  ...
  pkgPayload = (void *)((uint8_t *)pMsgIn + SapHandlerDataStructureOffSet);

  Copy2Bytes(pkgPayload->aNwkAddress, NlmeGetRequest(gNwkShortAddress_c));
  Copy8Bytes(pkgPayload->aIeeeAddress, NlmeGetRequest(gNwkIeeeAddress_c));
  pkgPayload->capability = NlmeGetRequest(gNwkCapabilityInformation_c);

  Zdp_GenerateDataReq(gDevice_annce_c, aDstAddress, (afToApsdeMessage_t *)pMsgIn, sizeof(zbEndDeviceAnnounce_t));
#else
(void)aDstAddress;
#endif
}
...
/****************************** Process Any Sibling Device *******************************/
  ...
  if (pNTE->deviceProperty.bits.relationship == gNeighborIsSibling_c)
  {
    ...
    {
      /*
        Update the network address in the NT, if the device is no longer our
        neighbor it will be expired by the NT aging mechanism.
      */
