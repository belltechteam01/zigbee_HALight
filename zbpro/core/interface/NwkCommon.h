...
#define IncrementNwkSequenceNumber()                  ZbBeeStackNwkGlobals(gNibSequenceNumber)++

#define IncrementNwkTxTotal()                         ZbBeeStackNwkGlobals(gNibTxTotal)++

#define IncrementNwkUpdateId()                        ZbBeeStackNwkGlobals(gNwkData.nwkUpdateId)++

#define ResetTxFailureCounterPerChild(iNteIndex)      ZbBeeStackNwkGlobals(gaNeighborTable[iNteIndex].transmitFailure) = 0
...
void NodeNotInParentLinkStatus(void);
#endif
...
* the NPDU. 
*
* Frame Control Field bits, see 053474r20 ZigBee Pro spec - Sec. 3.3.1.1
* Bits:
* 0-1: Frame type
...
*/
#define gRejoinRspTimerID                 ZbNwkPrivateData(gNwkTimerIDs[0])
#define gPollTimerID                      ZbNwkPrivateData(gNwkTimerIDs[1])
#define gLeaveTimerID                     ZbNwkPrivateData(gNwkTimerIDs[2])
#define gBroadcastJitterTimerID           ZbNwkPrivateData(gNwkTimerIDs[3])
#define gBroadcastDeliveryTimerID         ZbNwkPrivateData(gNwkTimerIDs[4])
#define gPermitJoinTimerID                ZbNwkPrivateData(gNwkTimerIDs[5])
#define gRREQJitterTimerID                ZbNwkPrivateData(gNwkTimerIDs[6])
#define gRouteDiscoveryTimerID            ZbNwkPrivateData(gNwkTimerIDs[7])
#define gLinkStatusTimerID                ZbNwkPrivateData(gNwkTimerIDs[8])
#define gLinkStatusMultipleFramesTimerID  ZbNwkPrivateData(gNwkTimerIDs[9])
#define gAgingNTEntriesTimerID            ZbNwkPrivateData(gNwkTimerIDs[10])
#define gRouteRepliesTimerID              ZbNwkPrivateData(gNwkTimerIDs[11])
#define gExpireRouteEntriesTimerID        ZbNwkPrivateData(gNwkTimerIDs[12])
#define gNwkRejoinStateMachineTimerID     ZbNwkPrivateData(gNwkTimerIDs[13])
...
extern zbTmrTimerID_t gNwkTimerIDs[];
...
typedef struct nwkAddrList_tag {
  uint8_t       iCurrentRouter;
  uint8_t       cDestAddrListLength;
  zbNwkAddr_t   aDestAddrList[1];
} nwkAddrList_t;

#define gRejoinConfigParamsMask_EnableConfig          0x01
#define gRejoinConfigParamsMask_EnableLqiFiltering    (gRejoinConfigParamsMask_EnableConfig | 0x02)
#define gRejoinConfigParamsMask_EnableAddrFiltering   (gRejoinConfigParamsMask_EnableConfig | 0x04)
#define gRejoinConfigParamsMask_EnablePreviousParent  (gRejoinConfigParamsMask_EnableConfig | 0x08)

void UpdateNeighborRelationsTable(uint8_t iSrcNteIndex, linkStatusCommand_t * pLinkStatusCommand);

void NWK_UpdateNeighborRelationsTable(uint8_t iSrcNteIndex, linkStatusCommand_t * pLinkStatusCommand);

void ResetNeighborRelationsTable(void);

void NWK_ResetNeighborRelationsTable(void);

uint8_t GetNumberOfCommonNeighbors(uint8_t iNTIndex);

uint8_t NWK_GetNumberOfCommonNeighbors(uint8_t iNTIndex);

neighborTable_t* IsMyChild ( zbAddrMode_t addressMode, uint8_t *pAddress );

void ResetNeighborTableEntry ( neighborTable_t *pNeighborTableEntry );
#ifdef __cplusplus
}
#endif
#endif /* _NWK_COMMON_H_ */
