...
typedef void ( *pfTsTaskEventHandler_t )( tsEvent_t );
...
/*---------------------------------------------------------------------------
 * NAME: TS_SetInstance
 * DESCRIPTION: Beestack instance change for internal tasks 
 *              Called from beestack rtos task before using task module.
 * PARAMETERS:  IN: pointers to tasks tables and task number
 * RETURN: None
 * NOTES: none
 *---------------------------------------------------------------------------*/
void TS_SetInstance(tsTaskTableEntry_t* pTaskTable, tsTaskID_t* pPriorityTaks, uint8_t maxNoTasks);
...
void TS_ClearEvent
  ...
  tsEvent_t events                        /* IN: Which event(s) to clear. */
  );
...
void TS_SendEvent
  ...
  tsEvent_t events                        /* IN: Event flag(s) to send. */
  );
#ifdef __cplusplus
}
#endif
