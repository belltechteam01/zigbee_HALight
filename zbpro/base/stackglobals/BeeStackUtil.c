...
const uint8_t maRandomNumbers[] = { 0x22, 0x18, 0x16 };
...
void BUtl_CreateExtendedAddress( void )
{
  ...
  if(Cmp8BytesToFs( NlmeGetRequest(gNwkIeeeAddress_c))) {
    ...
    ieee address */
    if( !Cmp8BytesToZero( ZbBeeStackNwkGlobals(gNwkData.aExtendedAddress) ) ) {
      NlmeSetRequest( gNwkIeeeAddress_c, ZbBeeStackNwkGlobals(gNwkData.aExtendedAddress) );
  ...
  Copy8Bytes(ZbBeeStackNwkGlobals(gNwkData.aExtendedAddress), NlmeGetRequest(gNwkIeeeAddress_c));
...
void BUtl_SetReceiverOn(void) {
  ...
  if (ZbBeeStackGlobalsParams(mBUtl_ReceiverIsTurnedOn)) {
    return;
  }
...
#endif 
  ZbBeeStackGlobalsParams(mBUtl_ReceiverIsTurnedOn )= FALSE;
} /* BUtl_SetReceiverOff() */
...
{
  FLib_MemSet(pMsg,0xFE,gSmallMsgSize_d);
  MM_Free(pMsg);
}
...
{ 
  ...
  pMsg = MM_Alloc(size);
  if(pMsg)
    FLib_MemSet(pMsg,0xA1,gSmallMsgSize_d);
  return pMsg;
}
...
bool_t CanIHearThisShortAddress(zbNwkAddr_t aSourceAddress)
{
  ...
  ZbBeeStackGlobalsParams(giIcanHearLinkCost) = 0;

  /* nothing in table, assume we can hear all nodes */
  if(!ZbBeeStackGlobalsParams(gICanHearYouCounter))
    return TRUE;

  /* look for it in the table */
  for(iIndex=0; iIndex < ZbBeeStackGlobalsParams(gICanHearYouCounter); iIndex++)
  {
    if(IsEqual2Bytes(aSourceAddress, ZbBeeStackGlobalsParams(gaICanHearYouTable[iIndex])))
    {
      #if gICanHearYouTableCapability_d
        /* user want to override the incoming LQI */
        /* see NwkDataServices.c, ProcessMcpsDataIndication() */
        ZbBeeStackGlobalsParams(giIcanHearLinkCost) = ZbBeeStackGlobalsParams(gaICanHearYouLqi[iIndex]);
      #endif
      return TRUE;
    }
  }
...
)
{
  ...
  if(addressCounter > ZbStackTablesSizes(gICanHearYouMax))
    addressCounter = ZbStackTablesSizes(gICanHearYouMax);
	ZbBeeStackGlobalsParams(gICanHearYouCounter) = addressCounter;

  /* copy in the addresses and LQI overrides */
  FLib_MemCpy(ZbBeeStackGlobalsParams(gaICanHearYouTable), pAddressList, sizeof(zbNwkAddr_t) * addressCounter);
  FLib_MemCpy(ZbBeeStackGlobalsParams(gaICanHearYouLqi), pLqiList, addressCounter);
}
  ...
  )
{
  ...
  count = ZbBeeStackGlobalsParams(gICanHearYouCounter);
  if(count > maxElementsInDstTable)
    count = maxElementsInDstTable;

  /* return the */
  FLib_MemCpy(pDstTable, ZbBeeStackGlobalsParams(gaICanHearYouTable), count * sizeof(zbNwkAddr_t));
  return count;;
}
