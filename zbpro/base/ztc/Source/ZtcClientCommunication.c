...
extern uint8_t UartX_RxBufferByteCount(void);  
/*****************************************************************************
...
extern bool_t UartX_IsTxActive(void) ;
/*****************************************************************************
...
ztcPacketStatus_t ZtcComm_CheckPacket(void) 
{
  ...
  if (bytesReceived >= sizeof(gZtcPacketFromClient[0])) 
  {
    return framingError;
  }
   ...
   *  - checksum          1 byte
   */
  payloadLength = gpZtcPacketFromClient->structured.header.len;
   ...
   */
  if (payloadLength > sizeof(gpZtcPacketFromClient->structured.payload)) 
  {
    return framingError;
  }
  ...
  if (bytesReceived == payloadLength + sizeof(clientPacketHdr_t) + sizeof(checksum)) 
  {
    ...
    {
      checksum ^= gpZtcPacketFromClient->raw[i];
    }
    if (checksum == gpZtcPacketFromClient->structured.payload[payloadLength]) 
    {
...
extern bool_t UartX_GetByteFromRxBuffer(unsigned char *pDst);
/*****************************************************************************
...
{
  #define mNumOfZtcReadPktCallsWithRxBufEmpty_c 0xffff
  uint8_t byte;
  uint8_t status;

  static uint32_t ztcReadPktCallsWithRxBufEmpty = mNumOfZtcReadPktCallsWithRxBufEmpty_c;
  ...
  while (getByteFromRxBuffer(&byte) ) 
  {
    ...
    gpZtcPacketFromClient->raw[bytesReceived++] = byte;
...
static void ZtcComm_ZtcCheckNextBytes(void)
{
  uint8_t i;
    ...
    for (i = 0; i < bytesReceived; ++i) 
  	{
      if (gpZtcPacketFromClient->raw[i] == mStxValue_c) 
  		{
        bytesReceived     -= (i + 1);
        FLib_MemCpy(gpZtcPacketFromClient->raw, gpZtcPacketFromClient->raw + i + 1, bytesReceived);
                       ...
                       )
{
  ...
  //uint8_t*  pMsg = MSG_Alloc(length + 2); /* STX, data, checksum */
  uint8_t*  pMsg = MSG_Alloc(length + 3); /* STX, data, checksum, checksum */
  if(pMsg == NULL)
  {
    if(pfCallBack)
    {
  ...
  pMsg[0]           = mStxValue_c;
  FLib_MemCpy(&pMsg[1], (uint8_t*)pSrc, length);
  pMsg[length + 1]  = checksum + ztcBeeStackInstId;
  pMsg[length + 2]  = checksum ^ pMsg[length + 1];
  
  /* Send frame */
  //while (!transmit(pMsg, length + 2,(void (*)(unsigned char /*const*/ *))MM_Free)){}
  while (transmit(pMsg, length + 3,(void (*)(unsigned char /*const*/ *))MM_Free)){}
  /* select instance to transmit */
  //while (transmit(pMsg, length + 2,NULL));  
...
#endif /* gZtcNonBlockingFunctionality_c */
