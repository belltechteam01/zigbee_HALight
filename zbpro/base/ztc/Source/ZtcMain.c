...
volatile bool_t bZtcPacketToClientIsBusy=FALSE;

/* UART packet received from the external client, minus the UART envelope. */
clientPacket_t gZtcPacketFromClient[2];

clientPacket_t* gpZtcPacketFromClient;

bool_t gZtcCommTxBlocking = FALSE;

#define ZtcComm_WritePacketToClient(var)  ZtcComm_TransmitBufferBlock((uint8_t *)&gZtcPacketToClient, (var))
/******************************************************************************
*******************************************************************************
...
void Ztc_Task(uint8_t *pMsg, uint8_t instId) {
#if !gZtcIncluded_d
  (void)pMsg;
  (void)instId;
#else
  ...
  gpZtcPacketFromClient = (clientPacket_t*)((uint8_t*)pMsg);
  
  FLib_MemCpy(&gZtcPacketFromClient[instId],(uint8_t*)gpZtcPacketFromClient,sizeof(clientPacketHdr_t)+gpZtcPacketFromClient->structured.header.len);
  MSG_Free(pMsg);
  
  gpZtcPacketFromClient = &gZtcPacketFromClient[instId];
  ...
  opcodeGroup = gpZtcPacketFromClient->structured.header.opcodeGroup;
  ...
  if (opcodeGroup == gAfApsdeOpcodeGroup_c && 
      gpZtcPacketFromClient->structured.header.msgType != gApsdeDataReqMsgType_d)
  {
    ZtcApsde_Handler();
    return;
  }
  ...
  if (!pSAPInfo) {
    if (gpfTestClientToApplicationFuncPtr) {
      (*gpfTestClientToApplicationFuncPtr)((void *) &gpZtcPacketFromClient->raw[1]);
    } else {
  ...
  pMsgTypeInfo = pZtcMsgTypeInfoFromMsgType(pSAPInfo->pMsgTypeTable,
                                            *(pSAPInfo->pMsgTypeTableLen),
                                            gpZtcPacketFromClient->structured.header.msgType);
  if (!pMsgTypeInfo) {
  ...
  /* a large message. */
  pMsgToSAPHandler = (sapHandlerMsg_t *) MSG_Alloc(gMaxRxTxDataLength_c);
  ...
  dstDataLen = (*pCopyFunc)(pMsgToSAPHandler->structured.payload,
                            ...
                            gpZtcPacketFromClient->structured.payload,
                            gpZtcPacketFromClient->structured.header.len,
  ...
  /* originated with Ztc. */
  pMsgToSAPHandler->structured.header.msgType = gpZtcPacketFromClient->structured.header.msgType;
...
void Ztc_TaskInit(void) 
{
  ...
  ZtcComm_Init();
#if !gZtcNonBlockingFunctionality_c 
	...
	if (maZtcSAPModeTable[pSrcSAPInfo->modeIndex] == gZtcSAPModeHook_c)
	{
		if (pSrcSAPInfo->pMsgQueue)
		{
			...
			if (pSrcSAPInfo->opcodeGroup == gMlmeNwkOpcodeGroup_c) /* If the message came from MAC layer */
			{
				...
				{
					if (pMsg->msgData.scanCnf.resList.pPanDescriptorBlockList->panDescriptorList) /* If the internal list of the scan has somem data or point to some place */
						/* Free the internal list comming in the the Scan from the MAC (second block) */
						if(pMsg->msgData.scanCnf.resList.pPanDescriptorBlockList->pNext)
						  MSG_Free(pMsg->msgData.scanCnf.resList.pPanDescriptorBlockList->pNext);
						/* Free the internal list comming in the the Scan from the MAC (first block) */
						MSG_Free(pMsg->msgData.scanCnf.resList.pPanDescriptorBlockList);
				}
...
)
{
	...
	ZtcComm_WritePacketToClient(dstDataLen + sizeof(gZtcPacketToClient.structured.header)+ sizeof(clientPacketStatus_t));
...
/* ZTC is included */
  ...
  */
  ...
  while(pFragHdr)
  {
    ...
    /* send through ZTC */
    ZtcComm_WritePacketToClient(sizeof(gZtcPacketToClient.structured.header) + (sizeof(apsdeFragment_t) - 1) + dataSize);
...
static void ZtcApsde_Handler(void)
{
  ...
  msgType = gpZtcPacketFromClient->structured.header.msgType;
  switch(msgType)
  {
    ...
    case gZtcMsgTypeLoadFragment_c:
      ...
      fragLen = gpZtcPacketFromClient->structured.header.len - 1;
      ...
      FLib_MemCpy((uint8_t *)pFragBlock + sizeof(zbTxFragmentedHdr_t), 
          &(gpZtcPacketFromClient->structured.payload[1]), fragLen);
    break;

    /* set the fragmentation mask (for debugging and ZCP testing) */
    case gZtcMsgTypeSetFragAckMask_c:     
      ZbApsPrivateData(giFragmentedPreAcked) = gpZtcPacketFromClient->structured.payload[0];
    break;

    /* set the maximum OTA fragment length */
    case gZtcMsgTypeSetMaxFragmentLength_c:
      ApsmeSetRequest(gApsMaxFragmentLength_c, *gpZtcPacketFromClient->structured.payload);
    break;

    /* set the window size (1-8) */
    case gZtcMsgTypeSetWindowSize_c:
      ApsmeSetRequest(gApsMaxWindowSize_c, *gpZtcPacketFromClient->structured.payload);
    break;

    /* set the interframe delay 1-255 milliseconds */
    case gZtcMsgTypeSetInterframeDelay_c:
      ApsmeSetRequest(gApsInterframeDelay_c, *gpZtcPacketFromClient->structured.payload);
    break;
  ...
  ZtcComm_WritePacketToClient(sizeof(gZtcPacketToClient.structured.header)+ gZtcPacketToClient.structured.header.len);
}
...
void ZtcReq_Handler(void) {
  ztcMsgType_t msgType = gpZtcPacketFromClient->structured.header.msgType;
  index_t i;
...
void ZtcMsgAPSResetReqFunc(void) {
  ...
  apsmeMessage.msgData.resetTypeReq = gpZtcPacketFromClient->structured.payload[0];
...
void ZtcMsgDeregisterEndPointFunc(void) {
  ...
  pScratch = (ZtcGetEndPointDescriptionPkt_t *) gpZtcPacketFromClient->structured.payload;
...
void ZtcMsgGetEndPointDescriptionFunc(void) {
  ...
  pGetEndPointDescPkt = (ZtcGetEndPointDescriptionPkt_t *) gpZtcPacketFromClient->structured.payload;
...
void ZtcMsgGetEpMaxWindowSizeFunc(void)
{
  ...
  maxWindowSize = AF_GetEpMaxWindowSize(gpZtcPacketFromClient->structured.payload[0]);
...
void ZtcMsgSetApsOverrideApsEncryption(void)
{
  ZbApsPrivateData(gOverrideApsFrameEncryptionSettings_c.overrideUpdateDeviceEncryption) = 
    gpZtcPacketFromClient->structured.payload[0];
  ZbApsPrivateData(gOverrideApsFrameEncryptionSettings_c.updateDeviceEncryption) = 
    gpZtcPacketFromClient->structured.payload[1];
  ZbApsPrivateData(gOverrideApsFrameEncryptionSettings_c.overrideSecUpdateDeviceProcessing) = 
    gpZtcPacketFromClient->structured.payload[2];
  
  ZtcMsgSetCnfStatus(gZbSuccess_c);
}
#endif

void ZtcMsgSetApsDeviceKeyPairSetKeyInfo(void)
{
#if !gApsLinkKeySecurity_d
  ZtcError(gZtcRequestIsDisabled_c);
#else
  zbStatus_t  status = gZbSuccess_c;
  size_t index = gpZtcPacketFromClient->structured.payload[0];
  if(index >= ZbStackTablesSizes(giApsDeviceKeyPairCount)) 
    status = gZbFailed_c;
  else
  {
    ZbBeeStackApsGlobals(gaApsDeviceKeyPairSet[index].iKey) = gpZtcPacketFromClient->structured.payload[0];
    ZbBeeStackApsGlobals(gaApsDeviceKeyPairSet[index].keyType) = gpZtcPacketFromClient->structured.payload[1];    
  }
  ZtcMsgSetCnfStatus(status);
#endif
}
...
#endif
    {
      uint16_t firstByte = gpZtcPacketFromClient->structured.payload[0];
      uint16_t secondByte = gpZtcPacketFromClient->structured.payload[1];
      firstByte = firstByte | (secondByte << 8);
      ZbZdoPublicData(gZdoOrgPollRate) = firstByte;
      NlmeSetRequest(gNwkIndirectPollRate_c,  ZbZdoPublicData(gZdoOrgPollRate));
      (void)ZDO_NLME_ChangePollRate( ZbZdoPublicData(gZdoOrgPollRate));
    }
#endif  

  ZtcMsgSetCnfStatus(0);
}



void ZtcMsgSetRejoinConfigParams(void)
{
  if (gpZtcPacketFromClient->structured.header.len >= sizeof(ZbBeeStackNwkGlobals(gRejoinConfigParams)))
    FLib_MemCpy(&ZbBeeStackNwkGlobals(gRejoinConfigParams), gpZtcPacketFromClient->structured.payload, sizeof(ZbBeeStackNwkGlobals(gRejoinConfigParams)));
...
void ZtcMsgSetApsDeviceKey(void)
{
#if !gApsLinkKeySecurity_d
  ZtcError(gZtcRequestIsDisabled_c);
#else
  ...
  if(!KeySet_SetTableEntry(gpZtcPacketFromClient->structured.payload[0], &gpZtcPacketFromClient->structured.payload[1]))
    status = gZbFailed_c;  
...
void ZtcMsgGetApsDeviceKey(void)
{
#if !gApsLinkKeySecurity_d
  ZtcError(gZtcRequestIsDisabled_c);
#else
  ...
  gZtcPacketToClient.structured.header.len = sizeof(zbStatus_t) + sizeof(zbAESKey_t);
  BeeUtilZeroMemory(gZtcPacketToClient.structured.payload, gZtcPacketToClient.structured.header.len);
    
  if(!KeySet_GetTableEntry(gpZtcPacketFromClient->structured.payload[0], &gpZtcPacketFromClient->structured.payload[1]))
    status = gZbFailed_c;  
...
void ZtcMsgGetApsDeviceKeyPairSet(void)
{
  ...
  pIeeeAddress = (void *)&gpZtcPacketFromClient->structured.payload[0];
...
void ZtcMsgSetApsDeviceKeyPairSet(void)
{
  ...
  */
  ...
  SSP_ApsRegisterLinkKeyData(pIeeeAddress, *pKeyType, pKey);
...
void ZtcMsgModeSelectReqFunc(void) {
  uint8_t byte;
  index_t count;
  index_t dataLen = gpZtcPacketFromClient->structured.header.len;
  uint8_t i;
  index_t payloadIndex = 0;

  if (dataLen > 0) {
    gZtcCommTxBlocking = !!gpZtcPacketFromClient->structured.payload[payloadIndex++];
  }

  dataLen -= sizeof(gpZtcPacketFromClient->structured.payload[0]);

  count = (dataLen <= mZtcSAPModeTableLen ? dataLen : mZtcSAPModeTableLen);

  for (i = 0; i < count; ++i) {
    byte = gpZtcPacketFromClient->structured.payload[payloadIndex + i];
...
/****************************************************************************
 * ZtcOtaSupportCallback:
 *   - ota support callback 
 ****************************************************************************/
void ZtcOtaSupportCallback(void)
{
  //OtaSupportCalback(&gZtcPacketToClient);
}
...
void ZtcGetMaxApplicationPayload()
{
  ...
  FLib_MemCpy(&zbApsdeDataReqFromClient, &gpZtcPacketFromClient->structured.payload[0], ofsData);
  zbApsdeDataReqFromClient.pAsdu = NULL;
  /* ofsData+2 means 2 the txOptions comes after the two bytes of the pAsdu */
  zbApsdeDataReqFromClient.txOptions = gpZtcPacketFromClient->structured.payload[ofsData+2];
...
void ZtcMsgReadExtendedAdrReqFunc(void) {
  FLib_MemCpy( &gZtcPacketToClient.structured.payload[sizeof(clientPacketStatus_t)],
       ZbBeeStackGlobalsParams(aExtendedAddress), 8 );
...
void ZtcMsgRegisterEndPointFunc(void) {
  ...
  epWindowSize = gpZtcPacketFromClient->structured.header.len -1;
  epWindowSize = gpZtcPacketFromClient->structured.payload[epWindowSize];
...
void ZtcMsgSetEpMaxWindowSizeFunc(void)
{
  ...
  Ep = gpZtcPacketFromClient->structured.payload[0];

  /* Window size wil lbe always the second byte of the packet. */
  maxWindowSize = gpZtcPacketFromClient->structured.payload[1];
...
void ZtcMsgSetChannelReqFunc(void)
{
  ...
  channelNumber = gpZtcPacketFromClient->structured.payload[0];
  ...
  /* set the channel bit */
  BeeUtilZeroMemory(ZbBeeStackGlobalsParams(gSAS_Ram.aChannelMask), sizeof(ZbBeeStackGlobalsParams(gSAS_Ram.aChannelMask)));
  BeeUtilSetIndexedBit(ZbBeeStackGlobalsParams(gSAS_Ram.aChannelMask), channelNumber);
...
void ZtcMsgSetICanHearYouListFunc(void)
{
  ...
  pTable = (ztcICanHearYouTable_t *)gpZtcPacketFromClient->structured.payload;
  iCount = pTable->count;
...
void ZtcMsgSetPanIDReqFunc(void) {
  FLib_MemCpy(NlmeGetRequest(gNwkPanId_c),
              (void *) gpZtcPacketFromClient->structured.payload,
              sizeof(NlmeGetRequest(gNwkPanId_c)));
}                                       /* ZtcMsgGetPanIDReqFunc() */
#endif
...
{
  uint8_t  *pStartMode = (uint8_t *)gpZtcPacketFromClient->structured.payload;
  /*
...
void ZtcMsgStartNwkReqFunc(void) {
  uint8_t startMode = *(uint8_t *)gpZtcPacketFromClient->structured.payload;
  uint8_t event = 0;
...
{
  uint8_t  *pStopMode = (uint8_t *)gpZtcPacketFromClient->structured.payload;
  uint8_t  stopMask;
...
void ZtcMsgAddToAddressMapPermanentFunc(void)
{
 ...
 i  = APS_AddToAddressMapPermanent(
                             &gpZtcPacketFromClient->structured.payload[0],
                             &gpZtcPacketFromClient->structured.payload[sizeof(zbIeeeAddr_t)]
                             ); 
...
{
 APS_RemoveFromAddressMap(&gpZtcPacketFromClient->structured.payload[0]); 
 ZtcMsgSetCnfStatus(gZtcSuccess_c);
}
#if gZtcIncluded_d
...
{
  FLib_MemCpy(ZbBeeStackGlobalsParams(aExtendedAddress), gpZtcPacketFromClient->structured.payload,8);

  MacPhyInit_WriteExtAddress(ZbBeeStackGlobalsParams(aExtendedAddress));
#if gBeeStackIncluded_d  
  NlmeSetRequest(gNwkIeeeAddress_c, gpZtcPacketFromClient->structured.payload);
#endif
}

void ZtcMsgWriteNwkMngAddressReqFunc(void)
{
  NlmeSetRequest(gNwkManagerAddr_c, gpZtcPacketFromClient->structured.payload);
}
#endif
...
void ZtcSetJoinFilterFlag(void) {
  ZbZdoPrivateData(gZDOJoinMode) = gpZtcPacketFromClient->structured.payload[0];
}                          
#endif
#endif
/****************************************************************************/
...
void  ZtcMemoryReadBlock(void)
{
  ...
  FLib_MemCpy(&addr, gpZtcPacketFromClient->structured.payload, 4);

  nBytes = gpZtcPacketFromClient->structured.payload[sizeof(addr)];
  lcount = nBytes;
...
void  ZtcMemoryWriteBlock(void)
{
  ...
  nBytes = gpZtcPacketFromClient->structured.payload[sizeof(addr)];
  pIn = &gpZtcPacketFromClient->structured.payload[sizeof(addr)+1];
  pOut = addr;
  nRead = nWritten = 0;
  while (nRead++ < nBytes)
  {
...
void  ZtcMemoryWriteByte(void)
{
  ...
  FLib_MemCpy(&addr, gpZtcPacketFromClient->structured.payload, 4);  
  vIn = gpZtcPacketFromClient->structured.payload[sizeof(addr)];
  pOut = addr;
  nWritten = 0;
...
void  ZtcMemoryWriteWord(void)
{
  ...
  FLib_MemCpy(&addr, gpZtcPacketFromClient->structured.payload, 4);
  pIn = &gpZtcPacketFromClient->structured.payload[sizeof(addr)];
  pOut = addr;
  nRead = nWritten = 0;
...
void  ZtcPing(void)
{
  ...
  gZtcPacketToClient.structured.header.len = gpZtcPacketFromClient->structured.header.len;
  FLib_MemCpy(gZtcPacketToClient.structured.payload, gpZtcPacketFromClient->structured.payload,
              gpZtcPacketFromClient->structured.header.len );
...
void ZtcMsgSetPermissionsTableFunc(void) {
  SetPermissionsTable(gpZtcPacketFromClient->structured.payload[0], &(gpZtcPacketFromClient->structured.payload[1]));
}                                       /*  */
...
{
  if (gpZtcPacketFromClient->structured.payload[0])
    RemoveAllFromPermissionsTable();
  else
    gZtcPacketToClient.structured.payload[0] = RemoveDeviceFromPermissionsTable(&gpZtcPacketFromClient->structured.payload[1]);
  gZtcPacketToClient.structured.header.len = sizeof(clientPacketStatus_t); 
}                                       /*  */

void ZtcMsgAddDeviceToPermissionsTableFunc(void)
{
  gZtcPacketToClient.structured.payload[0] = AddDeviceToPermissionsTable(&(gpZtcPacketFromClient->structured.payload[0]), gpZtcPacketFromClient->structured.payload[sizeof(zbIeeeAddr_t)]);
  gZtcPacketToClient.structured.header.len = sizeof(clientPacketStatus_t); 
}                                       /*  */
...
void ZtcMsgSetHighLowRamConcentrator(void)
{
  ZbBeeStackNwkGlobals(gNwkHighRamConcentrator) = gpZtcPacketFromClient->structured.payload[0];
  gZtcPacketToClient.structured.payload[0] = gZtcSuccess_c;
  gZtcPacketToClient.structured.header.len = sizeof(clientPacketStatus_t); 
}
#endif

void ZtcMsgClearNeighborTableEntry(void)
{
  neighborTable_t* pNTE = NULL;
  
  gZtcPacketToClient.structured.payload[0] = gZtcSuccess_c;
  
  if(gpZtcPacketFromClient->structured.payload[0] == 0xFF
     && gpZtcPacketFromClient->structured.payload[1] == 0xFF)
  {
    pNTE = IsInNeighborTable(gZbAddrMode64Bit_c, &gpZtcPacketFromClient->structured.payload[2]);
  }
  else
  {
    pNTE = IsInNeighborTable(gZbAddrMode16Bit_c, &gpZtcPacketFromClient->structured.payload[0]);
  }
...
#if gZtcIncluded_d && gASL_EnableEZCommissioning_d
void ZtcMsgEZComissioning_Start(void)
{
  uint8_t ezMode = gpZtcPacketFromClient->structured.payload[2];
  EZCommissioningConfigData(gEZCommissioningPrimaryDeviceType) = gpZtcPacketFromClient->structured.payload[0];
  EZCommissioningConfigData(gEZCommissioningUseGroup) = gpZtcPacketFromClient->structured.payload[1];
  EZComissioning_Start(ezMode);
  gZtcPacketToClient.structured.payload[0] = gZtcSuccess_c;
  gZtcPacketToClient.structured.header.len = sizeof(clientPacketStatus_t);
}
#endif
...
#if gZtcIncluded_d && gASL_EnableZllTouchlinkCommissioning_d
void ZtcMsgZllTouchlinkComissioning_Start(void)
{
  zllTouchlink_Start_t touchlinkStart;
  touchlinkStart.deviceType = gpZtcPacketFromClient->structured.payload[0];
  touchlinkStart.deviceRole = gpZtcPacketFromClient->structured.payload[1];
  touchlinkStart.functionality = gpZtcPacketFromClient->structured.payload[2];
  gZtcPacketToClient.structured.payload[0] = ZllTouchlinkComissioning_Start(&touchlinkStart);
  gZtcPacketToClient.structured.header.len = sizeof(clientPacketStatus_t);
}

void ZtcMsgZllTouchlinkComissioning_Configure(void)
{
  zllTouchlink_Configure_t touchlinkConfig;
  touchlinkConfig.minRssi = gpZtcPacketFromClient->structured.payload[0];
  touchlinkConfig.deviceConfigStartIndex = gpZtcPacketFromClient->structured.payload[1];
  FLib_MemCpy(&touchlinkConfig.idenfityDuration, &gpZtcPacketFromClient->structured.payload[2], sizeof(touchlinkConfig.idenfityDuration));
  gZtcPacketToClient.structured.payload[0] = ZllTouchlinkComissioning_Configure(&touchlinkConfig);
  gZtcPacketToClient.structured.header.len = sizeof(clientPacketStatus_t);
}

void ZtcMsgZllTouchlink_GetListOfCommissionedDevices(void)
{
  zllTouchlink_GetCommDevices_t getCommDevices;
  getCommDevices.startIndex = gpZtcPacketFromClient->structured.payload[0];
  getCommDevices.count = gpZtcPacketFromClient->structured.payload[1];
  gZtcPacketToClient.structured.payload[0] = ZllTouchlinkComissioning_GetListOfCommDevices(&getCommDevices, &gZtcPacketToClient.structured.payload[1]);
  gZtcPacketToClient.structured.header.len = sizeof(clientPacketStatus_t) + 1 + (gZtcPacketToClient.structured.payload[2])*sizeof(zllTouchlink_RemoteDeviceTable_t);
}

void ZtcMsgZllTouchlink_RemoveEntry(void)
{
#if gASL_EnableZllCommissioning_Initiator_d &&gInterPanCommunicationEnabled_c
  ZllTouchlink_RemoveEntryFromRemoteDeviceTable(gpZtcPacketFromClient->structured.payload);
  gZtcPacketToClient.structured.payload[0] = 0x00;
  gZtcPacketToClient.structured.header.len = sizeof(clientPacketStatus_t);
#else  
  gZtcPacketToClient.structured.payload[0] = 0x01;
  gZtcPacketToClient.structured.header.len = sizeof(clientPacketStatus_t);
#endif 
}
#endif
