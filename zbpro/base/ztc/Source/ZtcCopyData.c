  ...
  )
{
  ...
  FLib_MemCpy(maScanChannels,&pPktScanReq->scanChannels, sizeof(pPktScanReq->scanChannels)); 
  ...
  {            
    if(pMsgScanCnf->resultListSize <= aScanResultsPerBlock) 
    {
       FLib_MemCpy(pPkt + hdrLen, 
                  (void *) (&pMsgScanCnf->resList.pPanDescriptorBlockList->panDescriptorList), 
                  arrayLen);                                    
    } 
    else
    {  
       FLib_MemCpy(pPkt + hdrLen, 
          ...
          FLib_MemCpy(pPkt + hdrLen + elementLen*aScanResultsPerBlock + 1, 
                     (void *) (&pMsgScanCnf->resList.panDescriptorList->pNext->descriptorList), 
