...
bool_t gSendPartitionData = TRUE;
/******************************************************************************
*******************************************************************************
* Private prototypes
*******************************************************************************
...
const zclAttrDef_t gaPartitionClusterAttrDef[] = {  
  {gZclAttrIdPartitionMaxIncomingTransferSize_c,  gZclDataTypeUint16_c,    gZclAttrFlagsInRAM_c | gZclAttrFlagsRdOnly_c, sizeof(int16_t), (void *)MbrOfs(zclPartitionAttrs_t,maxInTrsSize)},
  {gZclAttrIdPartitionMaxOutgoingTransferSize_c,  gZclDataTypeUint16_c,    gZclAttrFlagsInRAM_c | gZclAttrFlagsRdOnly_c, sizeof(int16_t), (void *)MbrOfs(zclPartitionAttrs_t,maxOutTrsSize)},
  {gZclAttrIdPartitionPartitionedFrameSize_c,  gZclDataTypeUint8_c, gZclAttrFlagsInRAM_c, sizeof(int8_t), (void *)MbrOfs(zclPartitionAttrs_t,partitionedFrameSize)},
  {gZclAttrIdPartitionLargeFrameSize_c,  gZclDataTypeUint16_c, gZclAttrFlagsInRAM_c, sizeof(int16_t), (void *)MbrOfs(zclPartitionAttrs_t,largeFrameSize)},
  {gZclAttrIdPartitionNumberOfACKFrame_c,  gZclDataTypeUint8_c, gZclAttrFlagsInRAM_c, sizeof(int8_t), (void *)MbrOfs(zclPartitionAttrs_t,noOfAckFrame)},
  {gZclAttrIdPartitionNACKTimeout_c,  gZclDataTypeUint16_c, gZclAttrFlagsInRAM_c | gZclAttrFlagsRdOnly_c, sizeof(int16_t), (void *)MbrOfs(zclPartitionAttrs_t,nackTimeout)},
  {gZclAttrIdPartitionInterframeDelay_c,  gZclDataTypeUint8_c, gZclAttrFlagsInRAM_c, sizeof(int8_t), (void *)MbrOfs(zclPartitionAttrs_t,interframeDelay)},
  {gZclAttrIdPartitionNumberOfSendRetries_c,  gZclDataTypeUint8_c, gZclAttrFlagsInRAM_c | gZclAttrFlagsRdOnly_c, sizeof(int8_t), (void *)MbrOfs(zclPartitionAttrs_t,noOfSendRetries)},
  {gZclAttrIdPartitionSenderTimeout_c,  gZclDataTypeUint16_c,   gZclAttrFlagsInRAM_c | gZclAttrFlagsRdOnly_c, sizeof(int16_t), (void *)MbrOfs(zclPartitionAttrs_t,senderTimeout)},
  {gZclAttrIdPartitionReceiverTimeout_c,  gZclDataTypeUint16_c, gZclAttrFlagsInRAM_c | gZclAttrFlagsRdOnly_c, sizeof(int16_t), (void *)MbrOfs(zclPartitionAttrs_t,receiverTimeout)}  
 
};

const zclAttrSet_t gaPartitionClusterAttrSet[] = {
  {gZclAttrIdPartitionSet_c, gaPartitionClusterAttrDef, NumberOfElements(gaPartitionClusterAttrDef)}
};

const zclAttrSetList_t gPartitionClusterAttrSetList = {
  NumberOfElements(gaPartitionClusterAttrSet),
  gaPartitionClusterAttrSet
};

const zclCmd_t gaZclPartitionClusterCmdReceivedDef[]={
  // commands received 
  gZclPartitionCmdRx_TransferPartFrame_c,	
  gZclPartitionCmdRx_ReadHandshakeParam_c,	
  gZclPartitionCmdRx_WriteHandshakeParam_c	
};
const zclCmd_t gaZclPartitionClusterCmdGeneratedDef[]={
  // commands generated 
  gZclPartitionCmdTx_MultipleACKs,
  gZclPartitionCmdTx_ReadHandshakeParamRsp_c
};

const zclCommandsDefList_t gZclPartitiontClusterCommandsDefList =
{
   NumberOfElements(gaZclPartitionClusterCmdReceivedDef), gaZclPartitionClusterCmdReceivedDef,
   NumberOfElements(gaZclPartitionClusterCmdGeneratedDef), gaZclPartitionClusterCmdGeneratedDef
};
      ...
      if (pReq != NULL) {
        ...
        *(uint16_t*)pReq->partitionId = Native2OTA16(pPartitionEvent->partitionedFrameId);      
...

        {
          ...
          if(pPartitionEvent->partitionedFrameId == 0x00)
        	  buffer[1] = pPartitionEvent->zclTransactionId;
...
) 
{
  ...
  for (i = 0; i < gZclPartitionMaxRegisteredFrames_c; i++) {
    ...
    if (!pFrameEntry->isFree && 
        timerId == *(zbTmrTimerID_t *)((uint8_t *)&pFrameEntry->state + timerOffset))
      return i;
  }
  return gInvalidTableIndex_c;
}
...
) 
{
  ...
  if (blockComplete || sendNacks) {
    ...
    /* restart nack Timer */
    if (!sendNacks)
      ZbTMR_StartSingleShotTimer(pRxFrameState->nackTimer, OTA2Native16(pRxFrameAttrs->nackTimeout), 
...
) 
{
  ...
  if (entryIdx != gInvalidTableIndex_c) {
    ...
    ZbTMR_StartSingleShotTimer(pTxFrameState->senderTimer, 
                             OTA2Native16(pFrameEntry->frame.partitionAttrs.senderTimeout), 
                             ZCL_PartitionSenderTimeoutExpired);
                             
    if (restartTimer)
      ZbTMR_StartSingleShotTimer(timerId, pTxFrameAttrs->interframeDelay, ZCL_PartitionTxTimerHandler);
  }
}
...
) 
{
  ...
  ZbTMR_FreeTimer(gZclPartitionRxFrameTable[frameIdx].state.Rx.receiverTimer);
  
  /* cancel timers */
  ZbTMR_FreeTimer(gZclPartitionRxFrameTable[frameIdx].state.Rx.nackTimer);
  

  
  /* free Ack buffer */
  MSG_Free(gZclPartitionRxFrameTable[frameIdx].state.Rx.pAckList);
  gZclPartitionRxFrameTable[frameIdx].state.Rx.pAckList = NULL; 
...
{
  /* cancel timers */
  ZbTMR_FreeTimer(gZclPartitionTxFrameTable[frameIdx].state.Tx.senderTimer);
  
  ZbTMR_FreeTimer(gZclPartitionTxFrameTable[frameIdx].state.Tx.txTimer);
  
  /* free resend buffer */
  MSG_Free(gZclPartitionRxFrameTable[frameIdx].state.Tx.pListResend);
  gZclPartitionRxFrameTable[frameIdx].state.Tx.pListResend = NULL; 
...
) 
{
  ...
  if (frameIdx != gInvalidTableIndex_c) {
    ...
    /* restart TX timer */
    ZbTMR_StartSingleShotTimer(pTxFrameState->txTimer,  
...
//zclPartitionCmdMultipleACK16bit_t* pMultipleACKFrame16Bit = (zclPartitionCmdMultipleACK16bit_t*)pMultipleACKFrame;
  ...
  if (frameIdx != gInvalidTableIndex_c) {
    ...
    if (asduLength == 0) {
      ...
      pTxFrameState->pListResend = NULL; 
      
      /* if the full frame has been sent */
      if ((pTxFrameState->currentFrameIndex == OTA2Native16(pFrameEntry->frame.frameCount)) || 
          (pTxFrameState->currentStartFrame + pFrameEntry->frame.partitionAttrs.noOfAckFrame >= OTA2Native16(pFrameEntry->frame.frameCount)))
      { 
...
) 
{
  ...
  if (entryIdx != gInvalidTableIndex_c) {
    ...
    pFrameEntry->state.Rx.nackTimer = ZbTMR_AllocateTimer();
    pFrameEntry->state.Rx.receiverTimer = ZbTMR_AllocateTimer();
...
)
{
    ...
    if  (command == gZclPartitionCmdRx_TransferPartFrame_c) {
      ...
      result =  (pFrame->frameControl&gZclFrameControl_DisableDefaultRsp)?gZbSuccess_c:gZclSuccessDefaultRsp_c;;
    } 
...
) 
{
  ...
  if (entryIdx != gInvalidTableIndex_c) {
    ...
    /* initialize timers first so that we catch a frame alloc error early */
    pFrameEntry->state.Tx.txTimer = ZbTMR_AllocateTimer();
    pFrameEntry->state.Tx.senderTimer = ZbTMR_AllocateTimer();
    ...
    for (i = 0; i < NumberOfElements(gaPartitionClusterAttrDef); i++) {
      ...
      if (i != 0x02 &&
          ...
          i != 0x06) 
          continue;
      currentWriteParamRecord->attrId = Native2OTA16(pAttrDef->id);
      currentWriteParamRecord->attrType = pAttrDef->type;
      currentWriteParamRecord->attrLen = ZCL_GetDataTypeLen(pAttrDef->type, pAttrDef->data.pData, pAttrDef);
    ...
    /* start Tx timer */
    ZbTMR_StartSingleShotTimer(pFrameEntry->state.Tx.txTimer, pFrameAttrs->interframeDelay + 100, ZCL_PartitionTxTimerHandler);
...
) 
{
  ...
  for(i=0; i < NumberOfElements(gaPartitionClusterAttrDef); ++i)
  {
    zclAttrDef_t const *pCurrentAttrDef = &(gaPartitionClusterAttrDef[i]);
 ...
 )
{
  ...
  if (entryIdx != gInvalidTableIndex_c) {
    ...
    *pAttrLen = ZCL_GetDataTypeLen(pAttrDef->type, pAttrDef->data.pData, pAttrDef);
    return (void *)pAttr;    
  } else {
