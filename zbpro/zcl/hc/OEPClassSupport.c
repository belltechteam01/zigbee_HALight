  ...
  switch(pInApdu->choice)
  {
    ...
    case oepObjCfgChoice_GetObjectDescriptorReq:
    {
      ...
      if((csResult == gOepCsSucess_d)&&(pObject != NULL))
      {
        ...
        (void)oepCsDumpObjectDescriptor(pObject, &pObjectDescriptor->attrCount, (uint16_t*)&pApdu->apduLen);
    ...
    case oepObjCfgChoice_GetAttributeReq:
    {
      ...
      csResult = oepCsDumpAttribute(pGetAttributeReq->objHandle, pGetAttributeReq->attrType, (uint8_t*)(&pGetAttributeRsp->status), (uint16_t*)&pApdu->apduLen);
      if(csResult == gOepCsSucess_d)
      {
