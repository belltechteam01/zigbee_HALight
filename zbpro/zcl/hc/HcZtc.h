...
typedef PACKED_STRUCT Oem11073ZtcFrame_tag {
 ztcMsgType_t opCodeId;
 zclApdu_t *pApdu;
} Oem11073ZtcFrame_t;

typedef PACKED_STRUCT Oem11073Ztc_OpCodeIdMapEntry_tag {
  uint16_t oemFrameId;
  ztcMsgType_t opCodeId;
...
typedef PACKED_STRUCT prstGeneric_tag
{
  uint16_t apduChoice; 
  uint16_t apduLength; 
  uint16_t prstLength; 
...
typedef PACKED_STRUCT oepGenericFrame_tag
{
  uint16_t choice; 
  uint16_t length; 
  uint8_t  data[1];
...
typedef PACKED_STRUCT hcZtcMessage_tag {
  uint8_t msgType;
  uint8_t msgLen;
  uint8_t data[1];
} hcZtcMessage_t;

typedef PACKED_STRUCT hcZtcObjCfgFrame_tag {
  uint8_t nextFragmentPresent;
  zbEndPoint_t endPoint;
  uint8_t objCfgFrame[1];
} hcZtcObjCfgFrame_t;

typedef PACKED_STRUCT hcZtcFrame_tag {
  uint8_t nextFragmentPresent;
 ...
 typedef PACKED_STRUCT hcZtcFragmentContinuationFrame_tag {
  uint8_t nextFragmentPresent;
  uint8_t fragmentData[1];
} hcZtcFragmentContinuationFrame_t;

typedef PACKED_STRUCT oepGetObjectsRsp_tag {
  uint8_t  objCount;
  uint8_t  objHandlesTypesLists[1];
} oepGetObjectsRsp_t;

typedef PACKED_STRUCT oepSetNewHandleReq_tag {
  oepHandle_t oldHandle;
  oepHandle_t newHandle; 
} oepSetNewHandleReq_t;

typedef PACKED_STRUCT oepSetNewHandleCnf_tag {
  oepStatus_t status;
} oepSetNewHandleCnf_t;

typedef PACKED_STRUCT oepGetObjectDescriptorReq_tag {
  oepHandle_t objHandle;
} oepGetObjectDescriptorReq_t;

typedef PACKED_STRUCT oepGetObjectDescriptorRsp_tag {
  oepHandle_t objHandle;
  uint8_t attrCount;
  uint8_t attrTypeList[1];
} oepGetObjectDescriptorRsp_t;

typedef PACKED_STRUCT oepGetAttributeReq_tag {
  oepHandle_t objHandle;
  oidType_t attrType;
} oepGetAttributeReq_t;

typedef PACKED_STRUCT oepGetAttributeRsp_tag {
  oepHandle_t objHandle;
  oidType_t attrType;
  oepStatus_t status;
  uint8_t valueLen;
...
typedef PACKED_STRUCT oepSetAttributeReq_tag {
  oepHandle_t objHandle;
  oidType_t attrType;
  uint8_t  valueLen;
  uint8_t  value[1];
...
typedef PACKED_STRUCT oepSetAttributeCnf_tag {
  oepHandle_t objHandle;
  oidType_t attrType;
  oepStatus_t status;
...
typedef PACKED_STRUCT oepSetAttributeEnabledReq_tag {
  oepHandle_t objHandle;
  oidType_t attrType;
  oepAttributeEnabled_t isEnabled;
} oepSetAttributeEnabledReq_t;

typedef PACKED_STRUCT oepSetAttributeEnabledCnf_tag {
  oepHandle_t objHandle;
  oidType_t attrType;
  oepStatus_t status;
} oepSetAttributeEnabledCnf_t;

typedef PACKED_STRUCT hcZtcSendDataReport_tag {
  uint16_t invokeId;
  uint16_t dataReqId;
  uint16_t scanReportNo;
} hcZtcSendDataReport_t;

typedef PACKED_STRUCT hcZtcSendAssocReleaseReq_tag {
  uint8_t dstAddr[2];
  uint8_t  dstEndPoint;
  uint16_t reason;
} hcZtcSendAssocReleaseReq_t;

typedef PACKED_STRUCT hcZtcSendGetMdsRequest_tag {
  uint8_t dstAddr[2];
  uint8_t  dstEndPoint;
  uint16_t invokeId;
} hcZtcSendGetMdsRequest_t;

typedef PACKED_STRUCT oepObjCfgPayload_tag {
  uint16_t apduLen;
  uint16_t choice;
  PACKED_UNION {
