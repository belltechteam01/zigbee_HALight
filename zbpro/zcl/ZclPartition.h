...
#define gZclAttrIdPartitionSet_c                        0x00

#define gZclAttrIdPartitionMaxIncomingTransferSize_c    0x00
#define gZclAttrIdPartitionMaxOutgoingTransferSize_c    0x01  
#define gZclAttrIdPartitionPartitionedFrameSize_c       0x02
#define gZclAttrIdPartitionLargeFrameSize_c             0x03
#define gZclAttrIdPartitionNumberOfACKFrame_c           0x04
#define gZclAttrIdPartitionNACKTimeout_c                0x05
#define gZclAttrIdPartitionInterframeDelay_c            0x06
#define gZclAttrIdPartitionNumberOfSendRetries_c        0x07
#define gZclAttrIdPartitionSenderTimeout_c              0x08
#define gZclAttrIdPartitionReceiverTimeout_c            0x09
...
typedef PACKED_STRUCT zclPartitionCmdTransferPartitionedFrame_tag
{
...
typedef PACKED_STRUCT zclPartitionCmdMultipleACK_tag {
  zclPartitionAckOptions_t ackOptions;
  uint8_t firstFrameId;
  uint8_t nackId[1];
} zclPartitionCmdMultipleACK_t;

typedef PACKED_STRUCT zclPartitionCmdMultipleACK16bit_tag {
  zclPartitionAckOptions_t ackOptions;
  uint8_t firstFrameId[2];
  uint8_t nackId[1];
} zclPartitionCmdMultipleACK16bit_t;

typedef PACKED_STRUCT zclPartitionNackIdTable_tag {
  uint8_t sizeofNackId;
  uint8_t nackIdCount;
  uint8_t nackIds[1];
} zclPartitionNACKIdTable_t;

typedef PACKED_STRUCT zclPartitionBitwiseAckTable_t {
  uint8_t ackTable[gBitwiseAckBufferSize_c];
} zclPartitionBitwiseAckTable_t;

typedef PACKED_STRUCT zclPartitionCmdReadHandshakeParamFrame_tag
{
...
typedef PACKED_STRUCT zclPartitionCmdReadHandshakeParamReq_tag
{
...
typedef PACKED_STRUCT zclPartitionCmdWriteHandshakeParamRecord_tag
{
  zclAttrId_t   attrId;
...
typedef PACKED_STRUCT zclPartitionWriteHandshakeParamReq_tag
{
  uint8_t             transactionId;
  afAddrInfo_t        addrInfo;     /* IN: source/dst address, cluster, etc... */
...
typedef PACKED_STRUCT zclPartitionAttrs_tag
{
...
typedef PACKED_STRUCT zclPartitionTxFrameState_tag {
  uint8_t currentSendRetries;
  zbTmrTimerID_t txTimer;
  zbTmrTimerID_t senderTimer;
  bool_t sendingNacks;
...
typedef PACKED_STRUCT zclPartitionRxFrameState_tag {
  zbTmrTimerID_t nackTimer;
  zbTmrTimerID_t receiverTimer;
...
typedef PACKED_STRUCT zclPartitionedClusterFrameInfo_tag {
...
typedef PACKED_STRUCT zclPartitionedDataIndication_tag {
  uint8_t zclTransactionId;
...
typedef PACKED_STRUCT zclPartitionedClusterInfo_tag {
  zbClusterId_t clusterId;
...
typedef PACKED_STRUCT zclPartitionedClusterInfoRecord_tag {
  bool_t isFree;
...
typedef PACKED_STRUCT zclPartitionedClusterFrame_tag {
  bool_t isFree;
...
extern const zclAttrSetList_t gPartitionClusterAttrSetList;
